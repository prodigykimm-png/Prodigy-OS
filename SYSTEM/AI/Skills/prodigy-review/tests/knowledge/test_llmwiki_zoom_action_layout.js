...
async function openLongReview(harness) {
  ...
  const startup = await harness.evaluate(`window.KnowledgeExplorerHub.whenKnowledgeInboxSettled().then(()=>{const hub=window.KnowledgeExplorerHub,snapshot=hub.llmWikiLifecycleSnapshot(),root=document.querySelector('#knowledge-panel-llmwiki');return{status:snapshot.status,inbox:snapshot.inbox,providerCalls:window.__task21Stateful.providerCalls,writes:window.__task21Stateful.canonicalWrites+window.__task21Stateful.auditWrites,analyzeVisible:Boolean(root.querySelector('[data-intent-action="analyze_inbox"]')),reviewActions:root.querySelectorAll('[data-action="approve"],[data-action="reject"],[data-action="request-revision"]').length}})`);
  assert.equal(startup.inbox.state, "queued", JSON.stringify(startup));
  assert.equal(startup.inbox.pending, 1, JSON.stringify(startup));
  assert.equal(startup.providerCalls, 0, JSON.stringify(startup));
  assert.equal(startup.writes, 0, JSON.stringify(startup));
  assert.equal(startup.analyzeVisible, true, JSON.stringify(startup));
  assert.equal(startup.reviewActions, 0, JSON.stringify(startup));
  await harness.evaluate(`(()=>{const hub=window.KnowledgeExplorerHub;window.__zoomAnalyzeReady=new Promise((resolve,reject)=>{let settled=false;const cleanup=()=>{observer.disconnect();window.removeEventListener('task21-lifecycle-action',finish);clearTimeout(timer)},finish=()=>{const snapshot=hub.llmWikiLifecycleSnapshot();if(settled||snapshot.status!=='review'||!document.querySelector('#knowledge-panel-llmwiki [data-action="approve"]'))return;settled=true;cleanup();resolve({status:snapshot.status,inbox:snapshot.inbox,providerCalls:window.__task21Stateful.providerCalls,writes:window.__task21Stateful.canonicalWrites+window.__task21Stateful.auditWrites})},observer=new MutationObserver(finish),timer=setTimeout(()=>{cleanup();reject(new Error('ZOOM_MANUAL_ANALYZE_REVIEW_TIMEOUT:'+JSON.stringify({snapshot:hub.llmWikiLifecycleSnapshot(),providerCalls:window.__task21Stateful.providerCalls,actions:window.__task21Stateful.actions})))},15000);window.addEventListener('task21-lifecycle-action',finish);observer.observe(document.querySelector('#knowledge-panel-llmwiki'),{childList:true,subtree:true,attributes:true});finish()});return true})()`);
  await harness.renderedClick('#knowledge-panel-llmwiki [data-intent-action="analyze_inbox"]');
  const analyzed = await harness.evaluate("window.__zoomAnalyzeReady");
  assert.equal(analyzed.providerCalls, 1, JSON.stringify(analyzed));
  assert.equal(analyzed.writes, 0, JSON.stringify(analyzed));
  await harness.evaluate(`(()=>{const hub=window.KnowledgeExplorerHub;window.__zoomRepairReject=new Promise((resolve,reject)=>{const cleanup=()=>{observer.disconnect();window.removeEventListener('task21-lifecycle-action',finish);clearTimeout(timer)},finish=()=>{if(hub.llmWikiLifecycleSnapshot().status!=='cancelled')return;cleanup();resolve({providerCalls:window.__task21Stateful.providerCalls,writes:window.__task21Stateful.canonicalWrites+window.__task21Stateful.auditWrites})},observer=new MutationObserver(finish),timer=setTimeout(()=>{cleanup();reject(new Error('ZOOM_REPAIR_INITIAL_REJECT_TIMEOUT'))},10000);window.addEventListener('task21-lifecycle-action',finish);observer.observe(document.querySelector('#knowledge-panel-llmwiki'),{childList:true,subtree:true,attributes:true});finish()});return true})()`);
  await harness.renderedClick('#knowledge-panel-llmwiki [data-action="reject"]');
  const rejected = await harness.evaluate("window.__zoomRepairReject");
  assert.deepEqual(rejected, { providerCalls: 1, writes: 0 });
...
test("Given the real Knowledge review, When zoom reaches 200% and recovers, Then Korean actions reflow without clipping or collisions", { timeout: 240000 }, async () => {
  ...
  try {
    ...
    if (harness) {
      ...
      const cacheRoot = path.join(harness.runtime.vault, "SYSTEM/CACHE/llmwiki");
      if (fs.existsSync(cacheRoot)) for (const entry of fs.readdirSync(cacheRoot, { recursive: true })) {
        const absolute = path.join(cacheRoot, entry);
        if (fs.statSync(absolute).isFile() && entry.endsWith(".json")) expectedJson[path.relative(harness.runtime.vault, absolute)] = JSON.parse(fs.readFileSync(absolute, "utf8"));
      }
