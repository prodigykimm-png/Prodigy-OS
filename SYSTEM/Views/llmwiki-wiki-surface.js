(function (root) {
  ...
  function injectStyles(container) {
    ...
    style.textContent = [
      ".llmwiki-wiki-surface{display:grid;gap:12px;min-width:0;color:var(--text-normal);line-height:1.45}",
      ...
      ".llmwiki-wiki-surface input,.llmwiki-wiki-surface select{min-width:0;padding:8px;border:1px solid var(--background-modifier-border);border-radius:6px;background:var(--background-primary);color:var(--text-normal)}",
      ".llmwiki-wiki-surface button{padding:8px 12px;border:1px solid var(--background-modifier-border);border-radius:6px;background:var(--background-secondary);color:var(--text-normal);cursor:pointer}",
      ...
      ".llmwiki-wiki-surface__facet-rail,.llmwiki-wiki-surface__results,.llmwiki-wiki-surface__detail{padding:10px;border:1px solid var(--background-modifier-border);border-radius:8px;background:var(--background-primary)}",
      ...
      ".llmwiki-wiki-surface__result-title{font-weight:700;overflow-wrap:anywhere}",
      ".llmwiki-wiki-surface__result-meta,.llmwiki-wiki-surface__muted{color:var(--text-muted);font-size:.82em;overflow-wrap:anywhere}",
      ...
      "@media(max-width:760px){.llmwiki-wiki-surface__controls{grid-template-columns:1fr}.llmwiki-wiki-surface__content{grid-template-columns:1fr}.llmwiki-wiki-surface__detail{max-height:none}.llmwiki-wiki-surface__result-list{max-height:300px}}",
  ...
  function mountLlmWikiWikiSurface(options) {
    ...
    function render() {
      if (!rootEl) {
        empty(container);
        rootEl = createEl(container, "section", { attr: { class: "llmwiki-wiki-surface", "data-surface": "llmwiki-browse", "aria-label": "LLMWiki 탐색" } });
      ...
      const rail = createEl(content, "aside", { attr: { class: "llmwiki-wiki-surface__facet-rail", "data-component": "WikiFacetRail", "aria-label": "LLMWiki 필터" } });
      ...
      const resultsPanel = createEl(content, "section", { attr: { class: "llmwiki-wiki-surface__results", "data-component": "WikiResultList", "aria-label": "LLMWiki 결과" } });
      ...
      const detail = createEl(content, "article", { attr: { class: "llmwiki-wiki-surface__detail", "data-component": "WikiDetailPane", "aria-label": "LLMWiki 상세" } });
      renderDetail(detail);
    }
    ...
    const api = Object.freeze({
      ...
      destroy() { empty(container); rootEl = null; }
    });
    render();
    if (!snapshot) refresh();
    return api;
