(function (root) {
 ...
 function renderShell(container, renderers, options) {
  ...
  function createPanel(tab) {
    const panel = panelContainer.createDiv({
      attr: {
        ...
        hidden: tab.id !== currentTab ? "" : undefined,
      },
    });
  ...
  function renderRetry(panel, tabId, message, error, externalRetry) {
    panel.empty();
    const errorEl = panel.createDiv({ attr: { class: "workout-panel-error", role: "alert" } });
  ...
  function renderPanel(tabId) {
    ...
    panel.empty();
    const loading = panel.createEl("p", { text: "불러오는 중…", attr: { class: "workout-muted workout-panel-loading", role: "status", "aria-live": "polite" } });
    if (!renderer) {
