...
test("real Obsidian onboarding uses closure-backed create activation and visible fail-closed recovery", { timeout: 180000 }, async (t) => {
  ...
  try {
    ...
    for (const width of [1440, 390]) captures.push(await capture(harness, { name: "initial", width, expected: { inboxState: "protected", enabledPhases: [], activationFailure: false } }));
    captures.push(await capture(harness, { name: "initial-literature", width: 390, expected: { enabledPhases: [], frameLiterature: true } }));
    ...
    for (const width of [1440, 390]) captures.push(await capture(harness, { name: "create-enabled", width, expected: { enabledPhases: ["create"], activationFailure: false } }));
    captures.push(await capture(harness, { name: "create-enabled-literature", width: 390, expected: { enabledPhases: ["create"], frameLiterature: true } }));
  } finally {
  ...
  try {
    ...
    captures.push(await capture(harness, { name: "missing-closure", width: 390, expected: { enabledPhases: [], activationFailure: true } }));
  } finally {
    if (harness) cleanups.push(await harness.close());
  }
