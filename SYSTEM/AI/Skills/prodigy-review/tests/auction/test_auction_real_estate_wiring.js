...
test("Given the Auction dashboard loader, When source modules are loaded, Then learning precedes outcome approval", () => {
  ...
  ["prodigy-ai-consumer-manifests.js", "prodigy-ai-client.js", "prodigy-ai-consumer-runtime.js", "auction-real-estate-research-core.js", "auction-real-estate-source-runner.js", "auction-real-estate-research.js"].forEach((name) => assert.ok(index(name) >= 0));
  assert.ok(index("prodigy-ai-client.js") < index("auction-real-estate-research.js"));
});

test("Given the research summary flow, When AI is requested, Then it delegates to the shared consumer runtime", () => {
  const research = fs.readFileSync(path.join(ROOT, "SYSTEM/Views/auction-real-estate-research.js"), "utf8");
  assert.match(research, /ProdigyAIConsumerRuntime/u);
  assert.match(research, /consumerId: "auction\.research_summary"/u);
  assert.doesNotMatch(research, /AuctionAiProviderResolver|resolveAuctionAiProvider/u);
});
