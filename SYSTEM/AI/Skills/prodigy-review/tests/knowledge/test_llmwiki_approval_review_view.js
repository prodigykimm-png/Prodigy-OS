...
test("Given no supplied packet, When the Knowledge Hub mounts, Then it renders a truthful empty state without a synthetic review", async () => {
  ...
  assert.match(text, /검토할 실행이 없습니다\./);
  ...
  assert.equal(firstElement(result.container, "button", (node) => node.attr && node.attr["data-action"] === "open-review"), null);
...
test("Given a supplied packet and an injected writer, When the Hub approval action is activated, Then the normal browser path commits and replay shows duplicate", async () => {
  ...
  try {
    const result = await runHub({
      ...
      hubOptions: { commitApi: injectedCommit, commitOptions, buildCommitRequest: packetBound.build }
    });
  ...
  const open = firstElement(result.container, "button", (node) => node.attr && node.attr["data-action"] === "open-review");
  open.onclick({ preventDefault() {} });
  const checkbox = firstElement(result.container, "input", (node) => node.attr && node.attr["data-operation-id"]);
  checkbox.onclick();
  firstElement(result.container, "button", (node) => node.attr && node.attr["data-action"] === "approve-selected").onclick({ preventDefault() {} });
  const last = result.window.KnowledgeExplorerHub.approvalReview.state().lastResult;
  ...
  assert.match(collectText(result.container), /결정적 커밋이 완료되었습니다/);
  firstElement(result.container, "button", (node) => node.attr && node.attr["data-action"] === "approve-selected").onclick({ preventDefault() {} });
  const duplicate = result.window.KnowledgeExplorerHub.approvalReview.state().lastResult;
  assert.equal(duplicate.status, "duplicate");
  assert.match(collectText(result.container), /이미 같은 결정적 승인 결과가 확인되었습니다/);
  assert.equal(calls.length, 2);
  } finally {
...
test("Given a no-require VM without the canonical packet contract, When approval is clicked, Then commit fails closed without preview semantics", async () => {
  const packet = await packetFixture();
  const packetBound = await packetBoundCommitFixtures([packet]);
  const result = await runHub({ pages: buildPages(), approvalPacket: packet, hubOptions: { buildCommitRequest: packetBound.build } });
  assert.equal(result.window.require, undefined, "Hub VM must not install a global require shim");
  const open = firstElement(result.container, "button", (node) => node.attr && node.attr["data-action"] === "open-review");
  open.onclick({ preventDefault() {} });
  const checkbox = firstElement(result.container, "input", (node) => node.attr && node.attr["data-operation-id"]);
  checkbox.onclick();
  ...
  assert.match(collectText(result.container), /결정적 커밋을 진행하지 못했습니다/);
});
