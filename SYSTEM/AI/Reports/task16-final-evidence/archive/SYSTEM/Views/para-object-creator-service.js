...
(function (root) {
  ...
  function captureDependency(name, path) {
   ...
   */
  async function createArea(app, input, options, writeRequest) {
    ...
    var runtime = captureDependency("CaptureActionRuntime", "./capture-action-runtime.js");
    var writer = captureDependency("CaptureAuthorizedWriter", "./capture-authorized-writer.js");
    var payload = writeRequest && writeRequest.payload;
    var content = payload && payload.type === "area_family" && payload.name === name ? payload.content : buildAreaContent(name, options);
    if (!writeRequest || writeRequest.target_path !== filePath || typeof content !== "string"
      || writeRequest.payload_hash !== runtime.hashPayload(filePath, { type: "area_family", name: name, content: content })) {
      writer.assertCanonicalWriteRequest(writeRequest, null);
    }
    writer.assertCanonicalWriteRequest(writeRequest, null);

    if (hasCollision(app, filePath) || hasCollision(app, folder)) throw new Error("이미 존재하는 영역입니다: " + name);
   ...
   * Returns { ok, path, file } or throws on collision/invalid.
   */
  async function createDocumentation(app, input, options, writeRequest) {
    var validation = validateTitle(input);
    if (!validation.ok) throw new Error(validation.error);
    var name = validation.name;
    ...
    var writer = captureDependency("CaptureAuthorizedWriter", "./capture-authorized-writer.js");
    var payload = writeRequest && writeRequest.payload;
    var content = payload && payload.type === "documentation_note" && payload.name === name ? payload.content : buildDocumentationContent(name, options);
    if (!writeRequest || writeRequest.target_path !== filePath || typeof content !== "string"
      || writeRequest.payload_hash !== runtime.hashPayload(filePath, { type: "documentation_note", name: name, content: content })) {
      writer.assertCanonicalWriteRequest(writeRequest, null);
    }
    writer.assertCanonicalWriteRequest(writeRequest, null);
    if (hasCollision(app, filePath)) throw new Error("이미 존재하는 문서입니다: " + name);
    var file = await app.vault.create(filePath, content);
   ...
   */
  async function executeAction(actionId, app, title, options, writeRequest) {
    var id = clean(actionId).toLowerCase();
    var opts = options || {};
    ...
    if (id === "area") {
      return createArea(app, title, opts, writeRequest);
    }
    if (id === "documentation") {
      return createDocumentation(app, title, opts, writeRequest);
    }
    if (id === "literature") {
