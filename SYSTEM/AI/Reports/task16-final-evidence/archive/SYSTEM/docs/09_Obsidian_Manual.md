...
Object Creator, 새 People, Workout 가져오기, Daily의 missing-People, Knowledge PARA Area/Documentation은 첫 버튼에서 바로 저장하지 않는다. 첫 click/Enter/Space는 target, exact payload, proposal ID, SHA-256, current revision과 Confirm/Reject/Cancel이 있는 `human_review`를 연다. 그 화면에서 나중에 Confirm을 한 번 더 눌러야 저장된다.

표시 상태는 `capture_started` → `ai_proposal` → `human_review` → `human_confirmed` → `object_committed`, 종료 결과는 `rejected`, `cancelled`, `no_change`, `stale`, `conflict`, `error`다. Review 전이나 Reject/Cancel 뒤에는 저장이 0회다. People의 수동 메모·상호작용·수정·삭제와 기존 사람에 승인된 통찰 추가, Workout의 수동 운영 수정은 직접 작업이며, missing-person 생성과 import는 항상 위 Capture 검토를 사용한다.
