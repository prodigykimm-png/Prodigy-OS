  ...
  function open(app, initial, onSave) {
    ...
    class BlockModal extends obsidianModule.Modal {
      ...
      onOpen() {
        ...
        contentEl.createEl("p", { text: "하나의 의미 있는 경험만 기록합니다. 경험만 필수입니다.", attr: { style: "color:var(--text-muted);font-size:0.85em;margin:0 0 12px;" } });
        const makeField = (label, key, rows, placeholder) => {
          ...
          const area = contentEl.createEl("textarea", { attr: { rows: String(rows), style: "width:100%;min-height:48px;resize:vertical;padding:8px;border-radius:6px;border:1px solid var(--background-modifier-border);background:var(--background-primary);color:var(--text-normal);" } });
