...
async function testTimeoutSettingsAreConsumedByRequestPath() {
  ...
  // Use a quickly-failing requestUrl so the retry loop iterates
  // and the deadline check fires between attempts
  let chatAttempts = 0;
  try {
    await provider.requestChatText({
      app: {
      ...
      prompt: "fixture",
      contextEnvelope: validEnvelope(),
      sleep: async () => { await new Promise(function(r) { setTimeout(r, 1); }); }
    });
    assert.fail("should have thrown timeout");
  } catch (error) {
    assert.match(error.message, /시간이 초과/);
    assert.equal(chatAttempts, 1, "only 1 attempt before timeout fires");
    console.log("  PASS: timeout consumed — chatTimeoutMs: 1, error: \"" + error.message + "\", attempts: " + chatAttempts);
  }
  ...
  try {
    ...
    console.log("  PASS: structuredTimeoutMs consumed: \"" + error.message + "\", attempts: " + structAttempts);
  }
}
