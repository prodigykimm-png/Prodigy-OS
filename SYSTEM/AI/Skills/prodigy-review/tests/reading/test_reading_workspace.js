...
function main() {
  ...
  const responsive = readingView.mountResponsiveWorkspace({
    ...
    logicalWidth: tokens.RESPONSIVE_BREAKPOINTS.contentMax,
  ...
  responsive.setLogicalWidth(834);
  assert.equal(responsiveRoot.attr["data-reading-layout"], "medium");
  assert.equal(visiblePaneCount(), 1);
  responsive.setLogicalWidth(390);
  assert.equal(responsiveRoot.attr["data-reading-layout"], "compact");
  assert.equal(visiblePaneCount(), 1);
  responsive.setLogicalWidth(1068);
  assert.equal(responsiveRoot.attr["data-reading-layout"], "wide");
  assert.equal(visiblePaneCount(), 2);
  ...
  const manifests = require(path.join(ROOT, "SYSTEM/AI/Skills/prodigy-review/tests/shared/fixtures/workspace-manifest-v1.json")).entries;
  const homeRequired = manifests.home.required;
  assert.ok(homeRequired.includes("SYSTEM/Views/object-engine-core.js") || /HomeView|workspace-launcher/i.test(homeHub));
  ...
  const readingRequired = manifests.reading.required;
  assert.ok(readingRequired.includes("SYSTEM/Views/reading-workspace-core.js"));
  assert.ok(readingRequired.includes("SYSTEM/Views/reading-strategy-core.js"));
  assert.ok(readingRequired.includes("SYSTEM/Views/object-engine-core.js"));
  assert.ok(readingRequired.includes("SYSTEM/Views/reading-card.js"));
  ...
  assert.match(readingHub, /ProdigyTokens\.RESPONSIVE_BREAKPOINTS\.contentMax/);
