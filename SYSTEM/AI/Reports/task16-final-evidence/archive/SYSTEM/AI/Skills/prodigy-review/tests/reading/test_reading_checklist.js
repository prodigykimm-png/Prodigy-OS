...
function awaitAdapterCommit(adapter, expectedPath, trigger, timeoutMs = 2000) {
  const original = adapter.rename;
  return new Promise((resolve, reject) => {
    let settled = false;
    const restore = () => { adapter.rename = original; clearTimeout(guard); };
    adapter.rename = async (...args) => {
      try {
        const value = await original.apply(adapter, args);
        if (!settled && args[1] === expectedPath) { settled = true; restore(); resolve(expectedPath); }
        return value;
      } catch (error) {
        if (!settled) { settled = true; restore(); reject(error); }
        throw error;
      }
    };
    const guard = setTimeout(() => { if (!settled) { settled = true; restore(); reject(new Error("reading checklist commit signal timed out")); } }, timeoutMs);
    try { trigger(); } catch (error) { if (!settled) { settled = true; restore(); reject(error); } }
  });
}
...
async function testModalAndPersistence() {
  ...
  await awaitAdapterCommit(fixture.adapter, modal.data.store.pathFor(modal.data.id), () => firstInput.oninput());
  ...
  await awaitAdapterCommit(fixture.adapter, reopened.data.store.pathFor(reopened.data.id), () => reInput.oninput());
