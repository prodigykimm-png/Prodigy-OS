...
(function (root) {
  ...
  const REQUIRED_DEPENDENCIES = Object.freeze([
    ...
    "openExercisePopup", "openExerciseNoteSide"
  ]);
  const SUPPORT_DEPENDENCIES = Object.freeze([
    "empty", "iconButton", "recordStripText", "loadState", "startProgram"
  ]);
  const ALL_DEPENDENCIES = Object.freeze([...REQUIRED_DEPENDENCIES, ...SUPPORT_DEPENDENCIES]);
  let injectedDependencies = null;

  function configureDependencies(input) {
    const source = input || {};
    const missing = ALL_DEPENDENCIES.filter((name) => typeof source[name] !== "function");
    if (missing.length) throw new Error(`WorkoutModals dependencies are incomplete: ${missing.join(", ")}`);
    injectedDependencies = Object.freeze(Object.fromEntries(ALL_DEPENDENCIES.map((name) => [name, source[name]])));
    return injectedDependencies;
  }

  function dependency(name) {
    if (!injectedDependencies || typeof injectedDependencies[name] !== "function") {
      throw new Error(`WorkoutModals dependency is unavailable: ${name}`);
    }
    return injectedDependencies[name];
  }
  ...
  function renderTargetFilter(parent, options = {}) {
    ...
    if (options.label) wrap.createEl("label", { text: options.label });
    const select = wrap.createEl("select", { attr: { class: "workout-select", "aria-label": options.label || "부위 필터" } });
    select.createEl("option", { text: "전체 부위", attr: { value: "" } });
    (objects.EXERCISE_TARGETS || []).forEach((target) => select.createEl("option", { text: target.label, attr: { value: target.id } }));
    select.value = (objects.normalizeTarget && objects.normalizeTarget(options.initial)) || "";
    select.onchange = () => { if (options.onChange) options.onChange(select.value); };
    return {
      get: () => select.value,
      set: (value) => { select.value = (objects.normalizeTarget && objects.normalizeTarget(value)) || ""; },
      el: wrap,
    };
  }
  ...
  async function paintExerciseNoteBody(app, parent, name) {
    ...
    if (!file) {
      dependency("empty")(parent, "운동 노트가 없습니다. 노트를 만들면 본문이 여기에 표시됩니다.");
      return;
    }
    let source = "";
    try {
      ...
      dependency("empty")(parent, error.message || "본문을 읽지 못했습니다.");
      return;
    }
    const body = stripNoteFrontmatter(source).trim();
    if (!body) {
      dependency("empty")(parent, "본문이 비어 있습니다. 노트에 테크닉·팁 등을 적어 두세요.");
      return;
    }
  ...
  class ExerciseDetailModal extends ModalBase {
    ...
    onOpen() {
      ...
      pr.createEl("p", {
        text: dependency("recordStripText")(previous, best),
        attr: { style: "margin-top:var(--ke-space-3);" }
      });
      ...
      if (objects.exerciseObjectExists(this.app, this.name)) {
        button(actions, "노트 (사이드)").onclick = async () => {
          await dependency("openExerciseNoteSide")(this.app, this.name);
        };
  ...
  class AddExerciseToProgramModal extends ModalBase {
    ...
    onOpen() {
      ...
      const makeNote = this.contentEl.createEl("label", {
        attr: { style: "display:flex;align-items:center;gap:var(--ke-space-2);margin-top:var(--ke-space-3);font-size:var(--ke-type-label);" }
      });
      ...
      const doSave = async () => {
        ...
        try {
          ...
          const next = dependency("appendExerciseToProgram")(this.program, daySelect.value, {
            name: exerciseName,
          ...
          await dependency("persistProgram")(this.app, this.state, next);
          this.close();
          notice(target
        ...
        try {
          const draft = {
            ...
            days: [{
              ...
              exercises: [{
                name: exerciseName,
                prescribed_sets: dependency("makePrescribedSets")(setCount.value, reps.value, rpe.value)
              }]
            }]
          };
          const stored = await dependency("persistProgram")(this.app, this.state, draft);
          this.close();
  ...
  class ProgramEditorModal extends ModalBase {
    ...
    onOpen() {
      ...
      this.renderDays();
      const dayAdd = this.contentEl.createDiv({ attr: { class: "workout-inline-actions", style: "margin-top:var(--ke-space-2);" } });
      ...
      save.onclick = async () => {
        ...
        save.disabled = true;
        try {
          await dependency("persistProgram")(this.app, this.state, this.program);
          this.close();
    ...
    renderDays() {
      ...
      this.program.days.forEach((day, dayIndex) => {
        ...
        dependency("iconButton")(head, "×", "Day 삭제").onclick = () => {
    ...
    renderExercise(parent, day, exercise, index) {
      ...
      dependency("iconButton")(controls, "↑", "위로 이동").onclick = () => {
        if (index > 0) {
          [day.exercises[index - 1], day.exercises[index]] = [day.exercises[index], day.exercises[index - 1]];
          this.renderDays();
        }
      };
      dependency("iconButton")(controls, "↓", "아래로 이동").onclick = () => {
        if (index < day.exercises.length - 1) {
          [day.exercises[index + 1], day.exercises[index]] = [day.exercises[index], day.exercises[index + 1]];
          this.renderDays();
        }
      };
      dependency("iconButton")(controls, "⧉", "운동 복제").onclick = () => {
        const copy = clone(exercise);
        copy.id = `exercise_${core.stableHash(`${day.id}:${Date.now()}:${copy.name}`)}`;
        day.exercises.splice(index + 1, 0, copy);
        this.renderDays();
      };
      dependency("iconButton")(controls, "×", "운동 삭제").onclick = () => {
        day.exercises.splice(index, 1);
        this.renderDays();
      };
      if (objects.exerciseObjectExists(this.app, exercise.name)) {
        button(controls, "팝업").onclick = () => dependency("openExercisePopup")(this.app, exercise.name, this.state.sessions);
        button(controls, "노트").onclick = () => dependency("openExerciseNoteSide")(this.app, exercise.name);
      } else {
        button(controls, "노트 만들기").onclick = () => dependency("openExercisePopup")(this.app, exercise.name, this.state.sessions);
      }
      ...
      exercise.prescribed_sets.forEach((set, setIndex) => {
        ...
        dependency("iconButton")(row, "×", "세트 삭제").onclick = () => {
  ...
  class ImportProgramModal extends ModalBase {
    ...
    renderPreview(parent) {
      parent.empty();
      this.replacementReviews = this.replacementReviews || {};
      this.replacementFilters = this.replacementFilters || {};
      ...
      select.onchange = () => { this.replacementReviews = {}; this.replacementFilters = {}; render(); };
      render();
      ...
      button(actions, "프로그램 저장", true).onclick = async () => {
        try {
          ...
          const review = this.replacementReviews[sheetKey];
          const mapping = review ? review.mapping() : {};
          await importer.commitExerciseReplacementReview(candidate.program, mapping, async (program) => {
            const session = new Date().toISOString();
            let result = await captureWriter.saveProgram(
              this.app, createStore(this.app), objects, program,
              captureRuntime.humanConfirmation("workout-program-import", session),
              { action_id: "workout-program-import", locator: "ImportProgramModal:explicit-confirm" }
            );
            if (result.review_required) {
              const review = result.capture.record;
              const confirmation = await captureRuntime.requestReviewConfirmation(review, "workout-program-import");
              if (!confirmation) throw new Error("프로그램 저장을 취소했습니다.");
              result = await captureWriter.saveProgram(this.app, createStore(this.app), objects, program, confirmation, { action_id: "workout-program-import", locator: "ImportProgramModal:explicit-confirm", review });
            }
    ...
    renderReplacementUI(area, candidate) {
      area.empty();
      if (!importer || typeof importer.createExerciseReplacementReview !== "function") {
        area.createEl("p", { text: "운동 교체 기능을 불러오지 못했습니다. Obsidian을 다시 로드해 주세요.", attr: { class: "workout-muted" } });
        return;
      }
      const sheetKey = candidate.sheet_name || "0";
      if (!this.replacementReviews[sheetKey]) {
        this.replacementReviews[sheetKey] = importer.createExerciseReplacementReview(
          candidate.program,
          (query, _limit, options) => objects.searchExercises(this.app, query, 40, options),
        );
      }
      if (!this.replacementFilters[sheetKey]) this.replacementFilters[sheetKey] = {};
      const review = this.replacementReviews[sheetKey];
      const filters = this.replacementFilters[sheetKey];
      let unique;
      try { unique = review.sources(); } catch (_e) { unique = []; }
      ...
      const previewArea = area.createDiv({ attr: { class: "workout-import-details" } });
      const renderPreview = () => {
        previewArea.empty();
        const preview = review.preview();
        if (!preview.length) return;
        previewArea.createEl("h4", { text: "대체 미리보기" });
        preview.forEach((item) => {
          const block = previewArea.createDiv({ attr: { class: "workout-replace-label" } });
          block.createEl("strong", { text: `${item.source_name} → ${item.replacement.name}` });
          block.createEl("span", { text: `영향 Day: ${item.affected_days.map((day) => day.label).join(", ")}`, attr: { class: "workout-muted" } });
          const values = item.prescriptions.map((prescription) => {
            const sets = prescription.prescribed_sets.map((set) => `${set.set_number || "-"}세트 ${set.reps || "-"}회 · RPE ${set.rpe || "-"} · 휴식 ${set.rest || "-"}`).join(" / ");
            return `${prescription.day_label}: ${sets}`;
          }).join(" | ");
          block.createEl("span", { text: `보존되는 세트·횟수·RPE·휴식: ${values}`, attr: { class: "workout-muted" } });
        });
      };
      const renderSummary = () => {
        const count = review.preview().length;
        summary.empty();
        summary.createEl("span", { text: count ? `대체 ${count}개` : "교체 없음", attr: { class: count ? "workout-replace-count" : "workout-muted" } });
        if (count) {
          const resetBtn = summary.createEl("button", { text: "모두 원본으로", attr: { class: "prodigy-btn workout-button workout-chip-btn prodigy-configurator-chip", type: "button" } });
          resetBtn.onclick = () => { review.clearAll(); renderRows(); };
        }
        renderPreview();
      };
      const renderRows = () => {
        list.empty();
        unique.forEach((item) => {
          if (!filters[item.name]) filters[item.name] = { query: "", target: "" };
          const filter = filters[item.name];
          ...
          const selected = review.preview().find((entry) => entry.source_name === item.name);
          inputWrap.createEl("span", { text: selected ? `선택: ${selected.replacement.name}` : "원본 유지", attr: { class: selected ? "workout-replace-count" : "workout-muted" } });
          const target = inputWrap.createEl("select", { attr: { "aria-label": `${item.name} 부위 필터` } });
          target.createEl("option", { text: "전체 부위", attr: { value: "" } });
          (objects.EXERCISE_TARGETS || []).forEach((option) => target.createEl("option", { text: option.label, attr: { value: option.id } }));
          target.value = filter.target;
          const input = inputWrap.createEl("input", { attr: { type: "search", placeholder: "이름·부위·별칭 검색", value: filter.query, "aria-label": `${item.name} 대체 운동 필터` } });
          const candidates = inputWrap.createDiv({ attr: { class: "workout-inline-actions" } });
          const renderCandidates = () => {
            candidates.empty();
            const options = { target: filter.target, include_untargeted: false };
            const found = review.candidates(item.name, filter.query, options);
            if (!found.length) {
              candidates.createEl("span", { text: "일치하는 운동 없음", attr: { class: "workout-muted" } });
              return;
            }
            found.slice(0, 6).forEach((exercise) => {
              const choice = candidates.createEl("button", { text: exercise.name, attr: { type: "button", class: "prodigy-btn workout-button workout-chip-btn prodigy-configurator-chip" } });
              choice.onclick = () => {
                try { review.select(item.name, exercise); renderRows(); }
                catch (error) { notice(error.message || "대체 운동을 선택하지 못했습니다."); }
              };
            });
          };
          input.oninput = () => { filter.query = input.value.trim(); renderCandidates(); };
          target.onchange = () => { filter.target = target.value; renderCandidates(); };
          if (selected) {
            const clear = inputWrap.createEl("button", { text: "원본 유지", attr: { type: "button", class: "prodigy-btn workout-button workout-chip-btn prodigy-configurator-chip" } });
            clear.onclick = () => { review.clear(item.name); renderRows(); };
          }
          renderCandidates();
        });
        renderSummary();
  ...
  class SubstitutionModal extends ModalBase {
    ...
    onOpen() {
      ...
      const preview = this.contentEl.createDiv({ attr: { class: "workout-muted", style: "margin-top:var(--ke-space-2);" } });
      const renderPreview = () => {
        preview.empty();
  ...
  root.WorkoutModals = Object.freeze({
    ...
    REQUIRED_DEPENDENCIES, configureDependencies,
  });
