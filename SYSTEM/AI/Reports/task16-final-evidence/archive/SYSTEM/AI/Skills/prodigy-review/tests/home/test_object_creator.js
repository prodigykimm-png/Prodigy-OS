...
class CaptureTestEvent { constructor() { this.type = "click"; this.isTrusted = true; this.timeStamp = Date.now(); } }
const captureListeners = new Map();
global.Event = CaptureTestEvent;
global.document = { addEventListener(type, listener) { captureListeners.set(type, listener); }, removeEventListener(type, listener) { if (captureListeners.get(type) === listener) captureListeners.delete(type); } };
...
function human(action) {
  ...
  if (!global.__objectCreatorCaptureOwner) global.__objectCreatorCaptureOwner = runtime.mountTrustedInteractions({ root: global.document, document: global.document, scope: { track() {} }, session_id: "object-creator-test" });
  captureListeners.get("click")(new CaptureTestEvent());
  return runtime.humanConfirmation(action, `session-${action}`);
}
async function launchConfirmed(core, app, id, title) {
  const action = `object-creator-${id}`;
  const review = await core.launchExistingCreator(app, id, title, { humanConfirmation: human(action) });
  assert.equal(review.capture.record.state, "human_review");
  if (app.created) assert.equal(app.created.length, 0, "first event leaves zero canonical writes");
  return core.launchExistingCreator(app, id, title, { captureReview: review.capture.record, humanConfirmation: human(action) });
}
...
async function paraTests(core) {
  ...
  // Create Area via the Universal Object Creator entry point.
  {
    ...
    const res = await launchConfirmed(core, app, "area", "Green Data Center Initiative");
  ...
  // A target that appears after preflight but before the canonical mutation
  // is rejected by the real Object Creator mutation boundary.
  {
    const app = makeFakeApp();
    let targetReads = 0;
    const originalLookup = app.vault.getAbstractFileByPath;
    app.vault.getAbstractFileByPath = (p) => {
      if (/Race Area\/2\. Race Area\.md$/.test(p)) {
        targetReads += 1;
        if (targetReads >= 3) return { path: p };
      }
      return originalLookup(p);
    };
    await assert.rejects(
      () => launchConfirmed(core, app, "area", "Race Area"),
      /stopped: conflict/i
    );
    assert.equal(app.created.length, 0, "Object Creator TOCTOU collision writes zero files");
  }

  // Create Documentation via the Universal Object Creator entry point.
  {
    const app = makeFakeApp();
    const res = await launchConfirmed(core, app, "documentation", "Transparency Strategies");
    assert.equal(res.ok, true);
  ...
  // Collision writes nothing.
  {
    ...
    await assert.rejects(() => para.createArea(app, "Dup"), /Capture writer authority/);
    assert.equal(app.created.length, 0);
  }
  {
    const app = makeFakeApp(["PARA/RESOURCES/DOCUMENTATIONS/Dup Doc.md"]);
    await assert.rejects(() => para.createDocumentation(app, "Dup Doc"), /Capture writer authority/);
    assert.equal(app.created.length, 0);
  }
...
function main() {
  ...
  const fakeApp = {
    ...
    vault: {
      getAbstractFileByPath: (p) => fakeFiles.has(p) ? { path: p } : null,
      create: async (p, content) => { fakeFiles.set(p, content || ""); return { path: p }; },
      createFolder: async () => {},
      read: async (file) => fakeFiles.get(file.path || file) || ""
    }
  };
  ...
  return core.openExistingObject(fakeApp, {
    ...
    return launchConfirmed(core, fakeApp, "journal", "오늘 성찰");
  }).then((res) => {
    assert.equal(res.ok, true);
    ...
    const creatorTouchContract = (source) => [
      /\.poc-input\{[^}]*min-height:44px/s,
      /\.poc-type\{[^}]*min-height:44px/s,
      /\.poc-dup-actions button\{[^}]*min-height:44px/s,
      /\.poc-recent-item\{[^}]*min-height:44px/s,
      /\.poc-footer button\{[^}]*min-height:44px/s
    ...
    for (const mutation of [
      viewSrc.replace("min-height:44px", "min-height:36px"),
      viewSrc.replace("recentBox.createEl(\"button\"", "recentBox.createDiv("),
      viewSrc.replaceAll("box-shadow:none", "box-shadow:0 0 0 2px red")
    ]) {
      assert.equal(creatorTouchContract(mutation) && /recentBox\.createEl\("button"/.test(mutation) && /\.poc-input:focus-visible\{[^}]*box-shadow:none/s.test(mutation), false, "touch-target/keyboard/chrome mutation must be detected");
    }
