...
test("source-guide overview preserves source numerics without importing internal count summaries", () => {
  const source = "조리개 f5, 감도 ISO 100으로 촬영한다.";
  const rendered = api.renderDocument({
    document_kind: "source_guide",
    title: "영상 자료 안내",
    sections: [{
      heading: "촬영 조건",
      summary: "1개 claim의 원문 근거를 안내합니다.",
      paragraphs: [{ text: source }],
    }],
  }, "INBOX/영상.md");
  const gate = require("../../../../../Views/llmwiki-golden-quality-gate.js").evaluate({
    source_text: source,
    source_path: "INBOX/영상.md",
    document_text: rendered,
  });
  assert.equal(gate.ok, true, JSON.stringify(gate.issues));
  assert.deepEqual(gate.metrics.unsupported_numeric_tokens, []);
  assert.equal(gate.metrics.critical_token_recall, 1);
});
