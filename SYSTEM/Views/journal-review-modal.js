  ...
  function open(app, initial, onSave, options) {
    ...
    class ReviewModal extends obsidianModule.Modal {
      ...
      onOpen() {
        ...
        contentEl.createEl("p", { text: "오늘 무엇이 달라졌는지, 무엇을 배웠는지, 다음에 무엇을 시험할지 짧게 기록합니다.", attr: { style: "color:var(--text-muted);font-size:0.85em;margin:0 0 12px;" } });
        if (focusHints.length) {
          const focusBox = contentEl.createEl("div", { attr: { style: "margin:0 0 12px;padding:8px 10px;border-radius:8px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);font-size:0.84em;line-height:1.45;" } });
        ...
        const makeField = (label, key, rows, placeholder) => {
          contentEl.createEl("label", { text: label, attr: { style: "display:block;font-weight:600;margin:10px 0 4px;font-size:0.88em;" } });
          const area = contentEl.createEl("textarea", { attr: { rows: String(rows), style: "width:100%;min-height:72px;resize:vertical;padding:8px;border-radius:6px;border:1px solid var(--background-modifier-border);background:var(--background-primary);color:var(--text-normal);" } });
