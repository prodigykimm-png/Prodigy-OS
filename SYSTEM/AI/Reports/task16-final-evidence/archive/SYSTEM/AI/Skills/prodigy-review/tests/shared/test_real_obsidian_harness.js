...
function runtime(overrides = {}) {
  return Object.assign({
    executable: "<task-temp>/task/ObsidianTask.app/Contents/MacOS/Obsidian", bundle: "<task-temp>/task/ObsidianTask.app",
    profile: "<task-temp>/task/profile", vault: "<task-temp>/task/vault", nonce: "exact-nonce", port: 61234,
...
test("canonical cloned Obsidian launch requires exactly one mock keychain and a task-owned child HOME", () => {
  ...
  assert.deepEqual(validateLaunchContract(args, { HOME: home }, taskRoot, "<home>/inherited"), {
    mock_keychain_count: 1,
    child_home_task_owned: true,
    inherited_real_home: false,
  });
  assert.throws(() => validateLaunchContract(args.slice(1), { HOME: home }, taskRoot, "<home>/inherited"), /TASK13A_MOCK_KEYCHAIN_COUNT/u);
  assert.throws(() => validateLaunchContract([...args, "--use-mock-keychain"], { HOME: home }, taskRoot, "<home>/inherited"), /TASK13A_MOCK_KEYCHAIN_COUNT/u);
  assert.throws(() => validateLaunchContract(args, { HOME: "<home>/inherited" }, taskRoot, "<home>/inherited"), /TASK13A_CHILD_HOME_NOT_TASK_OWNED/u);
});
