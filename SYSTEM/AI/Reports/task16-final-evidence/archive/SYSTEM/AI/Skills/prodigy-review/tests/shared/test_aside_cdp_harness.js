...
test("wrong app PID cannot own the task endpoint", () => {
  const profile = "<task-temp>/task-profile";
  const bundlePath = "<task-temp>/AsideTask.app";
  const rows = [
  ...
  assert.throws(() => discoverTaskApplication({
    ...
    environmentText: `HOME=<task-temp>/home TMPDIR=<task-temp>/tmp --user-data-dir=${profile}`,
    home: "<task-temp>/home",
    temp: "<task-temp>/tmp",
...
test("LaunchServices parent may exit before independently parented app discovery", () => {
  ...
  const ownership = discoverTaskApplication({
    ...
    listeners: [73000],
    protectedSnapshot: { pids: new Set([33261]), ports: new Set([45103]), pgids: new Set([33261]) },
    environmentText: "HOME=<task-temp>/home TMPDIR=<task-temp>/tmp",
    home: "<task-temp>/home",
    temp: "<task-temp>/tmp",
  });
...
test("post-settlement ownership snapshot includes a late internal listener descendant", () => {
  ...
  const staleOwnership = discoverTaskApplication({
    ...
    environmentText: "HOME=<task-temp>/home TMPDIR=<task-temp>/tmp", home: "<task-temp>/home", temp: "<task-temp>/tmp",
  });
  ...
  const proof = proveRuntimeEndpointOwnership({
    ...
    home: "<task-temp>/home", temp: "<task-temp>/tmp", rows: [app, helper],
...
test("cleanup refuses a reused PID whose executable or start identity changed", () => {
  const profile = "<task-temp>/aside-task-nonce/profile";
  const runtime = {
    profile,
    ownedProcesses: [{ pid: 74000, pgid: 74000, startTime: "Tue Aug 11 01:02:03 2026", executable: "<task-temp>/AsideTask.app/Contents/MacOS/Aside" }],
  };
  const rows = [{
    pid: 74000,
    ppid: 1,
