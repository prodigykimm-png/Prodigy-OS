...
function snapshotWorkspace() {
  ...
  try {
    const git = require("node:child_process").spawnSync("git", ["status", "--short"], { cwd: ROOT, encoding: "utf8" });
    if (git.status !== 0) assert.equal(fs.existsSync(path.join(ROOT, ".git")), false, git.stderr);
    const skillFiles = [];
    const visit = (directory) => {
      for (const entry of fs.readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
        const absolutePath = path.join(directory, entry.name);
        if (entry.isDirectory()) visit(absolutePath);
        else if (entry.isFile()) skillFiles.push([path.relative(SKILL_DIR, absolutePath), fs.readFileSync(absolutePath, "base64")]);
      }
    };
    if (fs.existsSync(SKILL_DIR)) visit(SKILL_DIR);
    return {
      gitStatus: git.status === 0 ? git.stdout : "metadata-free-archive",
      skillFiles
    };
  } finally {
