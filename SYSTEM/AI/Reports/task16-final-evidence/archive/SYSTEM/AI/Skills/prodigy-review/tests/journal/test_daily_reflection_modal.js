...
async function testStateObserverAndCloseCancelPendingClassification() {
  let resolveProposal;
  const pendingProposal = new Promise((resolve) => { resolveProposal = resolve; });
  const states = [];
  let resolveLoading;
  const loading = new Promise((resolve) => { resolveLoading = resolve; });
  const harness = createModalHarness(async () => { throw new Error("confirm must not run"); }, {
    generatePromise: pendingProposal,
    onStateChange: (state) => {
      states.push(state);
      if (state.phase === "input" && state.busy) resolveLoading(state);
    }
  });
  try {
    const area = findElement(harness.instance.contentEl, (element) => element.tag === "textarea");
    area.value = "닫을 때 취소할 분류";
    area.oninput();
    await harness.button("일기 저장").onclick();
    const running = harness.button("AI 분류").onclick();
    await loading;
    assert.equal(harness.generatedInputs.length, 1);
    assert.ok(harness.generatedInputs[0].signal, "provider receives modal-owned cancellation");
    harness.instance.close();
    assert.equal(harness.generatedInputs[0].signal.aborted, true);
    resolveProposal(createProposal());
    await running;
    assert.equal(states.at(-1).phase, "closed");
    assert.equal(states.some((state) => state.phase === "confirm"), false, "a stale provider result cannot reopen review");
    assert.deepEqual(states.at(-1).selectedIds, []);
  } finally {
    harness.restore();
  }
}
