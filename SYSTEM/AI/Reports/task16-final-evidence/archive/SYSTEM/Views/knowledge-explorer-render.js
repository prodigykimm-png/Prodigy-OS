...
(function (root) {
  ...
  const SEARCH_CSS = `
.knowledge-explorer-shell>.knowledge-explorer-search{grid-column:1/-1}.knowledge-explorer-search{display:flex;flex-wrap:wrap;align-items:end;gap:8px;min-inline-size:0;inline-size:100%;max-inline-size:100%;margin-block-end:12px}.knowledge-explorer-search-label{display:flex;flex:1 1 16rem;flex-direction:column;gap:8px;min-inline-size:0;color:var(--text-muted)}input.knowledge-explorer-search-input{inline-size:100%;min-inline-size:0;min-block-size:44px;box-shadow:none;font:inherit}.knowledge-explorer-search-input:focus-visible,.knowledge-explorer-search-clear:focus-visible{outline:2px solid var(--ke-color-accent,var(--text-accent));outline-offset:2px}button.knowledge-explorer-search-clear{min-inline-size:44px;min-block-size:44px;box-shadow:none;white-space:normal}.knowledge-explorer-search-clear[disabled]{opacity:.5;cursor:not-allowed}.knowledge-explorer-search-status{flex:1 1 100%;min-inline-size:0;margin:0;color:var(--text-muted);overflow-wrap:anywhere}@media(forced-colors:active){.knowledge-explorer-search-input:focus-visible,.knowledge-explorer-search-clear:focus-visible{outline-color:Highlight}}
`;
  ...
  function button(parent, options) {
    const control = createEl(parent, "button", {
      ...
      attr: {
        type: "button",
        class: `${options.className || "knowledge-explorer-button"} prodigy-configurator-chip`,
  ...
  function pane(container, name, title, subtitle, surface) {
    const section = createEl(container, name === "domain" || name === "middle" ? "nav" : "section", { attr: { class: `knowledge-explorer-pane knowledge-explorer-${name}-pane prodigy-utility-card`, "data-pane": name, "aria-label": title, "aria-busy": name === "detail" && surface.state === "loading" ? "true" : "false" } });
  ...
  function renderSearch(container, query, model, surface, onQuery) {
    ...
    const input = createEl(label, "input", {
      attr: {
        type: "search",
        class: "knowledge-explorer-search-input prodigy-configurator-chip",
        "data-group": "search",
        "data-key": "global",
        "data-action": "search",
  ...
  function renderDetail(container, model, selection, surface, onAction, onOpenBeside, briefOptions, candidateInbox) {
    ...
    addBack(scroll, selection.focusPane, onAction, surface.state === "disabled");
    const status = createEl(scroll, "div", { attr: { class: "knowledge-explorer-status prodigy-status-line", role: "status", "aria-live": "polite", "data-state": surface.state } });
    ...
    assets.forEach((item) => {
      ...
      const card = createEl(assetSection, "article", { attr: { class: "knowledge-explorer-detail-card prodigy-utility-card" } });
