...
test("retained artifact scrub removes path-shaped and synthetic-secret test values", () => {
  ...
  try {
    ...
    fs.writeFileSync(file, "<home>/example/x <linux-home>/runner/x /private<task-temp>/x <task-temp>/x <file-uri>/x <worktree> <synthetic-secret> <synthetic-secret>\n");
    ...
    fs.writeFileSync(file, `${GREEN}<home>/example/private\n`); scrub(root, manifestFile);
...
test("visual screenshot paths retain only the verifiable persisted token", () => {
  const visual = { rows: [{ screenshot: { path: "<home>/example/private/run/row.png", sha256: "a".repeat(64) } }] };
...
test("retained gate/archive and live revision claims reject temporary-copy forgeries", (t) => {
  ...
  fs.writeFileSync(path.join(artifacts, "working.log"), `${RED_AUTHORITY}<home>/forged/path\n`);
