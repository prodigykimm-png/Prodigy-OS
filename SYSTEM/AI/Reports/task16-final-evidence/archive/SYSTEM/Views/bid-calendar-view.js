(function (root) {
  ...
  const tileMax = T.RESPONSIVE_BREAKPOINTS && T.RESPONSIVE_BREAKPOINTS.tileMax;
  if (!Number.isFinite(compactMax) || !Number.isFinite(tileMax)) throw new Error("Bid Calendar requires shared responsive tiers.");
...
.prodigy-bid-cal-popup-close {
  ...
  min-width: var(--ke-touch-target, 44px);
  padding: 4px 10px;
  border-radius: 6px;
  border: 1px solid var(--background-modifier-border);
...
.prodigy-bid-cal-popup-backdrop {
  ...
  inset: 0;
  z-index: 1000;
  background: color-mix(in srgb, var(--background-modifier-cover) 72%, transparent);
  display: flex;
...
@media (forced-colors: active) {
  .prodigy-bid-cal-cell.is-today,
  .prodigy-bid-calendar button:focus-visible,
  .prodigy-bid-cal-cell.in-month:focus-visible { outline: 2px solid CanvasText; }
}
@media (prefers-reduced-motion: reduce) {
  .prodigy-bid-calendar *, .prodigy-bid-cal-popup * { animation: none !important; transition: none !important; }
}
@media (min-width: ${tileMax + 1}px) {
  .prodigy-bid-cal-popup-backdrop {
    align-items: center;
  }
  .prodigy-bid-cal-popup {
...
`;
  ...
  function showDatePopup(options) {
    ...
    const backdrop = el(document.body, "div", { attr: { class: "prodigy-bid-cal-popup-backdrop", role: "dialog", "aria-modal": "true", "aria-label": title || formatDateLabel(date) } });
    ...
    function paint() {
      ...
      cells.forEach((cell) => {
        ...
        if (cell.inMonth && cell.date) {
          ...
          cellEl.setAttribute("aria-label", `${formatDateLabel(cell.date)} · 일정 ${cell.count}건`);
          const openCell = () => {
            const dayEvents = core.eventsForDate(events, cell.date);
            showDatePopup({ app, date: cell.date, events: dayEvents, pageByPath });
          };
          cellEl.onclick = openCell;
          cellEl.onkeydown = (event) => {
