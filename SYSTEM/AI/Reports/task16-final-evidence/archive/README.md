...
Node.js 24와 Python 3.12/uv 환경에서 로컬과 CI가 동일한 release gate를 실행합니다.
각 suite의 stdout/stderr는 생략하지 않으며, 하나가 실패해도 나머지 gate를 실행한 뒤 nonzero로 종료합니다.

```bash
bash SYSTEM/CI/run-release-gate.sh
```

This is the portable Node 24 gate used by Ubuntu CI. It reports macOS/Aside rows as not applicable rather than passing unavailable evidence. The final local macOS release authority is:

```bash
bash SYSTEM/CI/run-final-release.sh
```

That workflow requires fresh responsive, real-Obsidian visual256, and journeys8 evidence, sanitizes every persisted text artifact while retaining raw/persisted byte hashes and command-result bindings, derives the post-projection receipt, verifies it from explicit retained log/archive paths, then removes temporary artifacts. The receipt is intentionally excluded from the raw projection to avoid an unsatisfiable self-hash cycle. A 390px desktop window is labeled narrow desktop evidence only; physical iPhone remains `not_tested` / `not_proven`.

사용법과 failure-propagation self-test는 각각 `--help`, `--self-test`로 확인합니다.
Consolidation audit은 SHA-256 manifest로 고정된 tracked `SYSTEM/CI/fixtures/consolidation/` 입력을 사용하므로 clean checkout에서도 같은 gate를 재현합니다.
