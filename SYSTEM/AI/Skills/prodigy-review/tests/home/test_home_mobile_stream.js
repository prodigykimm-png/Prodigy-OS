...
class FakeElement {
  ...
  focus() {
    ...
    const listeners = this.focusListeners.splice(0);
...
function createApp({ mobile }) {
  ...
  return {
    ...
    workspace: {
      openLinkText(openedPath) {
        openedDestinations.push(/\.md$/i.test(openedPath) ? openedPath : `${openedPath}.md`);
      },
...
async function testCompactWorkspaceBarSingleRowContract() {
  ...
  const canonicalWorkout = WORKSPACE_REGISTRY.find("workout");
  ...
  assert.match(labelRule, /white-space:\s*normal/, "workspace labels wrap instead of being truncated");
  assert.doesNotMatch(labelRule, /text-overflow:\s*ellipsis|overflow:\s*(hidden|clip)/);
  ...
  buttons[0].onclick();
  assert.deepEqual(app.openedDestinations, [canonicalWorkout.path]);
  ...
  let sheetButtons = sheets[0].findAll((element) => element.hasClass("home-workspace-sheet-btn"));
  assert.deepEqual(
    sheetButtons.map((button) => button.attributes["data-workspace"]),
    WORKSPACE_REGISTRY.items().map((item) => item.id)
  );
  let prevented = 0;
  let focusReturnSignal = null;
  buttons[3].onceFocus((element) => { focusReturnSignal = element; });
  sheets[0].onkeydown({ key: "Escape", preventDefault() { prevented += 1; } });
  assert.equal(prevented, 1);
  assert.equal(focusReturnSignal, buttons[3], "Escape synchronously signals focus return to the subscribed opener");
  assert.equal(container.findAll((element) => element.hasClass("prodigy-bottom-sheet")).length, 0);
  assert.equal(buttons[3].attributes["aria-expanded"], "false");
  assert.equal(global.document.activeElement, buttons[3]);

  buttons[3].onclick();
  const reopenedSheet = container.findAll((element) => element.hasClass("prodigy-bottom-sheet"))[0];
  sheetButtons = reopenedSheet.findAll((element) => element.hasClass("home-workspace-sheet-btn"));
  const workoutSheetButton = sheetButtons.find((button) => button.attributes["data-workspace"] === "workout");
  workoutSheetButton.onclick();
  assert.deepEqual(app.openedDestinations, [canonicalWorkout.path, canonicalWorkout.path]);
