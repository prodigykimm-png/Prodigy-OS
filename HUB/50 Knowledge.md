...
KnowledgeExplorerHub.render = async ({ app: hubApp, dv: hubDv, container, obsidian: hubObsidian, mountContext }) => {
  ...
  try {
    ...
    const batchAnalyzer = batchIdentity && batchProvider ? window.LLMWikiBatchAnalyzer.createBatchAnalyzer({
      ...
      client: llmWikiControllerOptions.batchProvider ? null : llmWikiAIClient,
      identity: batchIdentity,
      cache: batchCache,
      coverage: batchCoverage,
    }) : null;
    const compactArtifactsFromHits = (hits, sourceOffset = 0) => hits.flatMap((hit) => {
      // Provider artifacts carry chunk-local spans and string claims. The local
      // materializer needs source-global spans, typed { text } claims, and its
      // established eight-item artifact envelope even when semantic v2 returns
      // more units for one source chunk.
      const items = JSON.parse(JSON.stringify(hit.artifact.items)).map((item) => {
      ...
      const artifacts = [];
      for (let start = 0; start < items.length || (start === 0 && items.length === 0); start += 8) {
        artifacts.push(Object.freeze({
          chunk_key: items.length > 8 ? `${hit.artifact.semantic_id}_part${Math.floor(start / 8) + 1}` : hit.artifact.semantic_id,
    ...
    const runCanonicalBatch = async ({ sources, candidates = [], signal, explicitRetry = false, retryIntentId = null, wholeSourceUnits = [], independentSources = false }) => {
      ...
      if (!analyzed.ok || !["review_ready", "resolved"].includes(analyzed.state)) return {
        ...
        source_results: analyzed.source_results || [], remaining_source_ids: analyzed.remaining_source_ids || [],
      };
      ...
      for (const sourceRow of sources) {
        const sourceResult = analyzed.source_results?.find(row => row.source_id === sourceRow.source_id);
        if (analyzed.source_results && (!sourceResult?.ok || !["review_ready", "resolved"].includes(sourceResult.state))) continue;
        const analysisText = sourceRow.scope ? sourceRow.extracted_text.slice(sourceRow.scope.start, sourceRow.scope.end) : sourceRow.analysis_text || sourceRow.extracted_text;
        ...
        const lookup = await batchCache.lookup(manifest, scope, { request_key: sourceResult?.request_key || analyzed.request_key });
        ...
        const materialized = await materializeInboxProposals({ artifacts, defer_document_proposals: independentSources, source: { source_id: sourceRow.source_id, source_path: sourceRow.source_path, content_hash: llmWikiHash.sha256(sourceRow.extracted_text) } });
        ...
        batchDocuments.push(...(materialized.documents || []));
      ...
      if (independentSources && batchDocuments.length) {
        const combined = inboxProposalMaterializer.materializeDocuments({ source: sourceGroups[0], sources: sourceGroups, documents: batchDocuments });
        if (!combined.ok) return { ok: false, reason: combined.reason, provider_calls: providerCalls };
        proposals.push(...combined.proposals);
        holds.push(...combined.holds);
        for (let index = 0; index < sourceGroups.length; index += 1) {
          const group = sourceGroups[index];
          sourceGroups[index] = Object.freeze({ ...group, proposals: combined.proposals.filter(proposal =>
            proposal.operation.source_citations.some(citation => citation.source_id === group.source_id)) });
        }
      }
      return { ok: true, provider_calls: providerCalls, replay_only: replayOnly, proposals, object_review_proposals: objectReviewProposals, holds, no_changes: noChanges, source_groups: sourceGroups, batch_id: analyzed.batch_id, job_id: analyzed.job_id, metrics: analyzed.metrics, artifacts_by_source: artifactsBySource, source_results: analyzed.source_results || [], remaining_source_ids: analyzed.remaining_source_ids || [], reason: analyzed.reason || "" };
    };
    ...
    if (!llmWikiRunController) {
      ...
      llmWikiRunController = window.LLMWikiRunController.createRunController({
        ...
        analyze_batch: async ({ command, signal, independent_sources }) => {
          ...
          const outcome = await llmWikiSession.bindings.runCanonicalBatch({ sources, candidates, signal, independentSources: independent_sources, explicitRetry: command.task13_explicit_retry === true, retryIntentId: command.task13_retry_intent_id || null });
    ...
    const readCanonicalDocuments = async () => {
      const read = await window.LLMWikiResurfacingReadAdapter.create().read({ app: appRef });
      if (!read.ok) throw Object.assign(new Error(`기존 Wiki를 확인하지 못했습니다: ${read.reason || "canonical_read_failed"}`), { code: read.reason || "canonical_read_failed" });
      const documents = read.rows.map((row) => ({ document_id: `canonical_${llmWikiHash.sha256(row.path).slice(0, 24)}`,
        path: row.path, title: row.title, statement: row.statement || "", summary: "",
        content: row.canonical_bytes, revision: row.canonical_revision }));
      const verifiedPaths = new Set(documents.map(document => document.path));
      // Legacy Knowledge is comparison evidence, never a finalized writer authority.
      // Excluding it here would turn cached analyses into duplicate new documents.
      for (const file of appRef.vault.getMarkdownFiles()) {
        if (!file.path.startsWith("ZETA/PERMANENT/") || verifiedPaths.has(file.path)
          || appRef.metadataCache.getFileCache(file)?.frontmatter?.type !== "knowledge") continue;
        const bytes = await appRef.vault.read(file);
        const document = window.KnowledgeCandidateStore.parseLifecycleDocument(bytes);
        if (document.schema_version === 2) continue;
        documents.push({ document_id: `canonical_${llmWikiHash.sha256(file.path).slice(0, 24)}`,
          path: file.path, title: document.title || file.path.split("/").pop().replace(/\.md$/u, ""),
          statement: document.statement || "", summary: document.summary || "", content: bytes, revision: llmWikiHash.sha256(bytes) });
      }
      return documents;
    };
    let canonicalDocuments = await readCanonicalDocuments();
    const discoveredRelatedCandidates = [];
    const planTargetBasis = () => llmWikiHash.sha256([...canonicalDocuments.map((doc) => `${doc.path}@${doc.revision || ""}`), ...discoveredRelatedCandidates.map((row) => `${row.path}@${row.revision || ""}`)].sort().join("|"));
    const rediscoverRelatedCandidates = async () => discoveredRelatedCandidates.splice(0, discoveredRelatedCandidates.length, ...(await Promise.all(appRef.vault.getMarkdownFiles()
    ...
    await rediscoverRelatedCandidates();
    const rebuildConfiguredCandidates = () => {
      configuredRelatedCandidates.splice(0, configuredRelatedCandidates.length, ...[
      ...
      configuredAllowedCandidateIds.splice(0, configuredAllowedCandidateIds.length, ...new Set([
    ...
    const configuredAllowedCandidateIds = [];
    rebuildConfiguredCandidates();
    const createInboxMaterializer = () => window.LLMWikiInboxProposalMaterializer.createInboxProposalMaterializer({
    ...
    let inboxProposalMaterializer = createInboxMaterializer();
    const materializeInboxProposals = async (input) => {
      ...
      return { ok: true, proposals: materialized.proposals, documents: materialized.documents || [], holds: materialized.holds || [], no_changes: materialized.no_changes || [], para_drafts: materialized.para_drafts || [], object_review_proposals };
    };
    ...
    let documentPlanQualityBlocked = null;
    const planQualityRetryState = { planHash: "", issues: [] };
    ...
    for (const job of Object.values(batchJobState.jobs)) for (const attempt of job.attempts || []) {
      if (attempt.kind === "correction") for (const tag of attempt.taxonomy_tags || []) {
        correctionSignals.record({ action: attempt.correction_action, taxonomy_tag: tag });
      }
    }
    ...
    const planEvidenceFromDocuments = () => canonicalDocuments.map((document) => ({
    ...
    let canonicalPlanEvidence = planEvidenceFromDocuments();
    const refreshCanonicalDocumentContext = async () => {
      canonicalDocuments = await readCanonicalDocuments();
      await rediscoverRelatedCandidates();
      rebuildConfiguredCandidates();
      canonicalPlanEvidence = planEvidenceFromDocuments();
      inboxProposalMaterializer = createInboxMaterializer();
      return { ok: true, canonical_documents: canonicalDocuments.length };
    };
    ...
    const matchPlanCandidates = (page, inventory, sourcePath) => {
      const claims = page.claim_ids.map((claimId) => inventory.claims.find((claim) => claim.claim_id === claimId)).filter(Boolean);
      const sourceTitle = String(sourcePath || "").split("/").pop().replace(/\.md$/u, "");
      const classified = window.LLMWikiCanonicalOverlap.classify({ page_title: `${sourceTitle} ${page.title}`, claims, canonical_documents: canonicalPlanEvidence });
      ...
      const evidence = classified.evidence.map((row) => ({
        ...
        revision: canonicalPlanEvidence.find((document) => document.candidate_id === row.candidate_id)?.revision || "",
    ...
    const buildPlanExecution = async (plan, inventory) => {
      ...
      const pages = plan.pages.map((page) => {
        ...
        const matched = page.target_candidate_ids.length
          ...
          : matchPlanCandidates(page, inventory, plan.source?.source_path);
    ...
    const planProposedTarget = (pageId) => {
      const row = planExecutionRow(pageId);
      const target = row?.candidate_evidence?.find(evidence => evidence.candidate_id === row.decision?.candidate_id && evidence.revision);
      return target && ["update", "no_change"].includes(row.decision.action)
        ? { path: target.path, revision: target.revision, action: row.decision.action } : null;
    };
    ...
    const planGroundedClaims = (claimIds, inventory) => {
      ...
      return claimIds.map((claimId) => {
        ...
        return { claim_id: claimId, text: claim?.text || claimId, citations,
          ...(claim?.original_topic_refs ? { original_topic_refs: claim.original_topic_refs } : {}) };
      });
    };
    ...
    const planReviewRows = (plan, inventory, compiledDocuments = []) => {
      ...
      const inventoryClaimById = new Map(inventory.claims.map((claim) => [claim.claim_id, claim]));
      const guideSections = plan.source_guide.sections.map((section) => ({
        heading: section.heading,
        paragraphs: planGroundedClaims(section.claim_ids, inventory)
          .filter((claim) => inventoryClaimById.has(claim.claim_id) && claim.text)
          .map((claim) => ({
            text: claim.text,
            claim_ids: [claim.claim_id],
            citation_ids: claim.citations.map((citation) => citation.citation_id),
            citations: claim.citations,
          })),
      }));
      const guideBodySections = guideSections.length === plan.source_guide.sections.length
        && guideSections.every((section) => section.paragraphs.length > 0)
        ? guideSections.map((section) => [`### ${section.heading}`, "", ...section.paragraphs.flatMap((paragraph) => [paragraph.text, ""])]).flat()
        : [];
      const guideDocumentBody = guideBodySections.length ? [
      ...
      const rows = [{
        ...
        summary_points: [
          plan.source_guide.overview,
          ...guideSections.flatMap((section) => section.paragraphs.map((paragraph) => paragraph.text)),
          ...(documentPlanQualityGaps.length
            ? [`추가 검토 후보(참고용) · ${documentPlanQualityGaps.slice(0, 3).map((gap) => gap.text || gap.evidence_quote).join(" · ")}`]
            : []),
        ],
        ...(guideDocumentBody ? { document_body: guideDocumentBody } : {}),
      ...
      for (const [pageIndex, page] of plan.pages.entries()) {
        rows.push({
          ...
          proposed_target: planProposedTarget(page.page_id),
          related_knowledge: (planExecutionRow(page.page_id)?.candidate_evidence || [])
            ...
            .map((evidence) => ({
              ...
              // 중복/충돌은 상대 문장을 봐야 판단할 수 있다: 겹치는 문장을 함께 넘긴다.
              covered_claims: (evidence.covered_claim_ids || [])
                .map((claimId) => inventory.claims.find((claim) => claim.claim_id === claimId)?.text)
                .filter(Boolean).slice(0, 3),
            })),
          destination: "none", review_state: reviewState, analysis_state: "complete",
          operation: planExecutionRow(page.page_id)?.decision?.action || page.operation_hint, title: page.title,
          summary_points: [
            page.purpose,
      ...
      for (const [documentIndex, document] of compiledDocuments.entries()) {
        const inventoryClaimIds = new Set((inventory?.claims || []).map((claim) => claim.claim_id));
        const requestedClaimIds = [];
        const unresolvedClaimRefs = [];
        for (const claim of document.claims || []) {
          if (claim.claim_id && inventoryClaimIds.has(claim.claim_id)) {
            requestedClaimIds.push(claim.claim_id);
            continue;
          }
          let resolvedAny = false;
          for (const derivedId of claim.derived_from_claim_ids || []) {
            if (inventoryClaimIds.has(derivedId)) {
              requestedClaimIds.push(derivedId);
              resolvedAny = true;
            } else if (!unresolvedClaimRefs.includes(derivedId)) unresolvedClaimRefs.push(derivedId);
          }
          if (claim.claim_id && !inventoryClaimIds.has(claim.claim_id) && !resolvedAny && !unresolvedClaimRefs.includes(claim.claim_id)) {
            unresolvedClaimRefs.push(claim.claim_id);
          }
        }
        const claimIds = [...new Set(requestedClaimIds)];
        const paragraphClaimIds = (document.paragraphs || []).flatMap((paragraph) => paragraph.claim_ids || []);
        const pageMemberIds = document.page_id ? (plan.pages.find((page) => page.page_id === document.page_id)?.claim_ids || []) : null;
        const sameClaimSet = (left, right) => {
          const leftIds = [...new Set(left)], rightIds = [...new Set(right)];
          return leftIds.length === rightIds.length && leftIds.every((claimId) => rightIds.includes(claimId));
        };
        const claimMismatch = unresolvedClaimRefs.length > 0
          || (pageMemberIds !== null && !sameClaimSet(claimIds, pageMemberIds))
          || !sameClaimSet(paragraphClaimIds.filter((claimId) => inventoryClaimIds.has(claimId)), claimIds)
          || paragraphClaimIds.some((claimId) => !inventoryClaimIds.has(claimId));
        rows.push({
          ...
          plan_page_id: document.page_id || "",
          draft_preview: true,
          ...(claimMismatch ? { review_blocked: { reason: "compiled_claim_mismatch", unresolved_claim_refs: unresolvedClaimRefs.slice(0, 8) } } : {}),
          ...
          proposed_target: planProposedTarget(document.page_id),
          ...
          ...(document.document_kind === "source_guide" ? {
            wiki_result: {
              ...
              possible_gap_count: documentPlanQualityGaps.length,
              hold_count: plan.pages.filter((page) => page.selected === false).length,
              guide_sections: document.sections,
              quality_status: documentPlanCompileResult?.quality_status || "unverified",
              quality_rewrite_count: Number(documentPlanCompileResult?.quality_rewrite_count || 0),
            },
          } : {}),
          ...
          operation: planExecutionRow(document.page_id)?.decision?.action || document.operation_hint, title: document.title,
          summary_points: [
    ...
    const runDocumentPlan = async (requestedSourcePath, runOptions = {}) => {
      await refreshCanonicalDocumentContext();
      ...
      const semanticUnits = window.LLMWikiEvidenceCandidates.createSemantic(scopedText);
      const sourceBytes = new TextEncoder().encode(scopedText).length;
      const fullSourceBytes = new TextEncoder().encode(extractedText).length;
      const contentHash = sourceRevision;
      ...
      const source = {
        ...
        ...(requestedScope ? { scope: { scope_id: requestedScope.scope_id || "", title: requestedScope.title || "", start: requestedScope.start, end: requestedScope.end, content_hash: llmWikiHash.sha256(scopedText) } } : {}),
      };
      ...
      const analyzed = await runCanonicalBatch({
        sources: [{ ...source, extracted_text: extractedText, ...(requestedScope ? { scope_start: requestedScope.start } : {}) }],
        candidates: [],
        wholeSourceUnits,
        explicitRetry,
        retryIntentId,
      ...
      const rawSourceHoldItems = (analyzed.artifacts_by_source.get(sourceId) || []).flatMap((artifact) => artifact.items
        .map((item, position) => ({ artifact, item, position }))
        .filter(({ artifact, item }) => artifact.outcome === "hold" || item.role === "hold" || item.claims.length === 0)
        .map(({ artifact, item, position }) => ({
          unit_id: `${artifact.chunk_key}_item${position}`,
          evidence_key: item.evidence_key,
          span: item.span,
        })));
      const sourceHoldItems = rawSourceHoldItems.map((hold, index) => ({
        ...hold,
        hold_id: analyzed.holds.find((row) => row.unit_id === hold.unit_id)?.hold_id || analyzed.holds[index]?.hold_id || hold.unit_id,
      }));
      const sourceCoverageAudit = window.LLMWikiQualitySignals.audit({
        source_path: sourcePath,
        source_length: extractedText.length,
        scope_start: requestedScope ? requestedScope.start : 0,
        scope_end: requestedScope ? requestedScope.end : extractedText.length,
        source_bytes: sourceBytes,
        semantic_units: semanticUnits,
        inventory_citations: inventoryResult.value.citations,
        holds: [...sourceHoldItems, ...analyzed.holds],
      });
      ...
      if (sourceCoverageAudit.blocks_approval) {
        ...
        return {
          ...
          source_units_covered: sourceCoverageAudit.covered_units,
          source_units_missing: sourceCoverageAudit.missing_units,
          source_units_held: sourceCoverageAudit.hold_units,
          source_units_duplicates: sourceCoverageAudit.duplicate_units,
          source_coverage_units: sourceCoverageAudit.units,
          map_provider_calls: analyzed.provider_calls, plan_provider_calls: 0,
          inventory_claims: inventoryResult.value.claims.length,
          existing_review_writes: 0, canonical_writes: 0, source_writes: 0, preview_writes: 0, writer_count: 0,
          write_counts: { source: 0, canonical: 0, preview: 0, review: 0 },
        };
      }
      const requestPlan = llmWikiControllerOptions.documentPagePlan || (async (request) => {
        const response = await window.ProdigyAIConsumerRuntime.requestStructured({
          ...
          consumerId: "wiki.page_plan",
          prompt: JSON.stringify({
            task: "Build a concise source guide and a reviewable page plan. The source guide must partition every claim exactly once into 1-16 sections with a short summary. Reusable claims must appear exactly once either in 0-20 topic pages or source_only_claim_ids. Judge each page by grounded reusable value and same-topic comparison: one meaningful supplement against an existing target is reviewable, and a coherent single-claim page is allowed. Prefer updating an allowlisted existing candidate when it is clearly the same topic. Never split a source or invent additional claims to satisfy a quota. Return claim ids and allowlisted candidate ids only; never return prose documents, paths, or write operations.",
            request,
          }),
          schema: window.LLMWikiDocumentReducer.PLAN_SCHEMA,
          ownerSessionId: `wiki-page-plan-${sourceId}`,
      ...
      const staleTerminal = !reusedPlan && !semanticReplan && Boolean(priorPlanSnapshot) && (priorPlanSnapshot.plan?.pages?.length || 0) === 0;
      let planned;
      if (reusedPlan) planned = { ok: true, value: reusablePlan, provider_calls: 0 };
      else if (staleTerminal) {
        const localDraft = window.LLMWikiDeterministicPagePlanner.plan({ inventory: inventoryResult.value });
        let localPlanned = null;
        if (localDraft.ok) {
          const localPlanner = window.LLMWikiDocumentReducer.createPagePlanner({
            allowedCandidateIds: configuredAllowedCandidateIds,
            requestPlan: async () => localDraft.value,
          });
          const localResult = await localPlanner.plan({ inventory: inventoryResult.value });
          if (localResult.ok && localResult.value.pages.length > 0) localPlanned = localResult;
        }
        if (!localPlanned) {
          const leftovers = inventoryResult.value.claims.filter((claim) => (claim.citation_ids || []).length > 0);
          if (leftovers.length) {
            const leadTitle = String(leftovers[0].text || "").replace(/\s+/gu, " ").trim().slice(0, 60) || "보완 근거 검토";
            const singletonDraft = {
              source_guide: {
                overview: "추가 검토가 필요한 근거를 원문 그대로 보존한다.",
                sections: [{ heading: "미검토 근거", summary: "아직 페이지에 오르지 않은 근거를 나열한다.",
                  claim_ids: leftovers.map((claim) => claim.claim_id) }],
                key_questions: [],
              },
              topic_pages: [{
                title: leadTitle,
                purpose: "단건 보완 근거를 검토한다.",
                claim_ids: leftovers.map((claim) => claim.claim_id),
                target_candidate_ids: [],
              }],
              source_only_claim_ids: [],
            };
            const singletonPlanner = window.LLMWikiDocumentReducer.createPagePlanner({
              allowedCandidateIds: configuredAllowedCandidateIds,
              requestPlan: async () => singletonDraft,
            });
            const singletonResult = await singletonPlanner.plan({ inventory: inventoryResult.value });
            if (singletonResult.ok && singletonResult.value.pages.length > 0) localPlanned = singletonResult;
          }
        }
        planned = localPlanned || await planner.plan({ inventory: inventoryResult.value });
      }
      ...
      documentPlanContext = {
        source, source_bytes: sourceBytes,
      ...
      if (!reusedPlan) await batchJobStore.savePlanSnapshot({
      ...
      documentPlanQualityBlocked = null;
      planQualityRetryState.planHash = "";
      planQualityRetryState.issues = [];
      ...
      return {
        ...
        source_bytes: documentPlanContext.source_bytes, full_source_bytes: documentPlanContext.full_source_bytes,
        source_coverage: sourceCoverageAudit.source_coverage,
        source_units_total: sourceCoverageAudit.total_units,
        source_units_covered: sourceCoverageAudit.covered_units,
        source_units_missing: sourceCoverageAudit.missing_units,
    ...
    const dispatchDocumentPlanAction = async (intent) => {
      ...
      const correctionAction = intent.action === "rename_page" ? "retitle_page" : intent.action === "toggle_page"
        ? before.pages.find(page => page.page_id === intent.page_id)?.selected === false ? "include_page" : "exclude_page" : intent.action;
      const correctionTags = window.LLMWikiCorrectionSignals.ALLOWED_ACTIONS.includes(correctionAction)
      ...
      await batchJobStore.savePlanSnapshot({
        ...
        review_event: { action: correctionAction, taxonomy_tags: correctionTags, ...(typeof intent.reason === "string" ? { reason: intent.reason } : {}) },
        inventory: documentPlanInventory, target_basis: planTargetBasis(),
      });
      recordPlanCorrection(correctionAction, correctionTags);
    ...
    const compileDocumentPlan = async (compileOptions = {}) => {
      ...
      if (documentPlanCompileResult?.restored && documentPlanCompileResult.quality_status === "draft"
        ...
        return {
          ...
          plan_hash: plan.plan_hash, quality_status: "draft", provider_calls: 0,
      ...
      const compiled = await compiler.compile({ inventory: documentPlanInventory, approved_plan: plan, execution: documentPlanExecution }, { allow_quality_retry: compileOptions.allow_quality_retry === true });
      if (!compiled.ok) {
        if (compiled.reason === "article_quality_review_required" && Array.isArray(compiled.articles)) {
          documentPlanQualityBlocked = { plan_hash: plan.plan_hash, articles: compiled.articles, issues: compiled.quality_issues || [] };
          planQualityRetryState.planHash = plan.plan_hash;
          planQualityRetryState.issues = (compiled.quality_issues || []).map((issue) => issue.code);
          try {
            const priorBlocked = batchJobStore.getPlanSnapshot(documentPlanContext.job_id);
            if (priorBlocked) await batchJobStore.savePlanSnapshot({ ...priorBlocked, quality_blocked: documentPlanQualityBlocked, plan_revision: priorBlocked.plan_revision + 1 });
          } catch (_) {}
          refreshReviewWorkbench();
        }
        return { ...compiled, provider_calls: articleProviderCalls };
      }
    ...
    let goldenRunSourceCoverage = null;
    ...
    const getGoldenWikiOrchestrator = () => {
      ...
      goldenWikiOrchestrator = window.LLMWikiGoldenWikiOrchestrator.create({
        ...
        runPlan: async (sourcePath, options) => {
          const result = await runDocumentPlan(sourcePath, options);
          goldenRunSourceCoverage = result.ok && documentPlanContext
            ? {
                identity: {
                  source_path: documentPlanContext.source.source_path,
                  source_revision: documentPlanContext.source.source_revision,
                },
                counters: { ...result.source_coverage },
              }
            : null;
          return result;
        },
    ...
    const runGoldenWiki = async (scope = null, runOptions = {}) => {
      ...
      activePromise = (async () => {
        try {
          ...
          const retainedCoverage = orchestrationResult.ok
            && goldenRunSourceCoverage
            && goldenRunSourceCoverage.identity.source_path === orchestrationResult.source_path
            && goldenRunSourceCoverage.identity.source_revision === orchestrationResult.source_hash
            ? goldenRunSourceCoverage : null;
          const result = retainedCoverage ? {
            ...orchestrationResult,
            source_coverage_identity: retainedCoverage.identity,
            source_coverage: retainedCoverage.counters,
          } : orchestrationResult;
          if (result.ok) {
            await refreshGoldenPreviewWorkbench();
            ...
            await prodigyWikiOperationStore.interrupt({ reason: result.reason || "golden_wiki_failed", resumable, stage: result.stage || "", ...((result.issues || result.metrics) ? { detail: { issues: result.issues, metrics: result.metrics } } : {}) });
            prodigyWikiController.dispatch({
              type: "interrupt",
              stage: result.stage || "",
              result,
    ...
    if (retainedPlanSnapshot) {
      ...
      documentPlanExecution = await buildPlanExecution(retainedPlanSnapshot.plan, retainedPlanSnapshot.inventory);
      ...
      documentPlanCompileResult = restoredDocuments.length ? {
        ...
        quality_status: restoredQuality.status === "invalid" ? "revalidation_required" : "draft",
    ...
    const openStoredDocumentPlan = async (jobId, options = {}) => {
      ...
      const sourceFile = appRef.vault.getAbstractFileByPath(snapshot.plan.source.source_path);
      if (!sourceFile || llmWikiHash.sha256(await appRef.vault.read(sourceFile)) !== snapshot.source_revision) {
        return { ok: false, reason: "source_revision_changed", writer_count: 0 };
      }
      const execution = await buildPlanExecution(snapshot.plan, snapshot.inventory);
      if (!execution.ok) return { ok: false, reason: execution.reason || "stored_plan_execution_failed", writer_count: 0 };
      documentPlanInventory = snapshot.inventory;
      documentPlanExecution = execution;
      if (snapshot.quality_blocked && snapshot.quality_blocked.plan_hash === snapshot.plan.plan_hash
        && Array.isArray(snapshot.quality_blocked.articles) && snapshot.quality_blocked.articles.length) {
        documentPlanQualityBlocked = snapshot.quality_blocked;
        planQualityRetryState.planHash = snapshot.plan.plan_hash;
        planQualityRetryState.issues = (snapshot.quality_blocked.issues || []).map((issue) => issue.code || issue);
      } else {
        documentPlanQualityBlocked = null;
        planQualityRetryState.planHash = "";
        planQualityRetryState.issues = [];
      ...
      const documents = Array.isArray(snapshot.compiled_documents) ? snapshot.compiled_documents.filter((document) =>
        Array.isArray(document.claims) && document.claims.length > 0 && document.claims.every((claim) => {
          if (claim.claim_id && (snapshot.inventory.claims || []).some((row) => row.claim_id === claim.claim_id)) return true;
          const derived = claim.derived_from_claim_ids || [];
          return derived.length > 0 && derived.every((id) => (snapshot.inventory.claims || []).some((row) => row.claim_id === id));
        })) : [];
      ...
      documentPlanCompileResult = documents.length ? { ok: true, documents,
        ...
        quality_status: quality.status === "invalid" ? "revalidation_required" : "draft",
    ...
    const runInboxBatch = ({ explicitRetry = false } = {}) => {
      ...
      if (!response || response.ok !== true) {
        ...
        const results = response?.source_results || [], remaining = response?.remaining_source_ids || [];
        const failed = results.length || sources.length - remaining.length;
        const state = settleInbox({ ...baseCounts, state: "blocked", recovery_variant: recoveryVariant,
          processed: results.length, succeeded: 0, failed, pending: remaining.length,
          source_results: results, remaining_source_ids: remaining,
          proposal_blocked: failed, proposal_state: "blocked", reason, message: "", object_review_proposals: [] });
      ...
      const sourceResults = response.source_results || [];
      const failedSources = sourceResults.filter(row => !row.ok || !["review_ready", "resolved"].includes(row.state));
      const remaining = response.remaining_source_ids || [];
      const succeeded = sourceResults.length ? sourceResults.length - failedSources.length : sources.length;
      const state = settleInbox({ ...baseCounts, state: failedSources.length || remaining.length ? "partial" : "complete",
        processed: sources.length - remaining.length, succeeded, failed: failedSources.length, pending: remaining.length,
        source_results: sourceResults, remaining_source_ids: remaining,
        pack_progress: { completed: packTotal, total: packTotal, current: packTotal }, proposal_pending: packets.length,
        proposal_complete: succeeded, proposal_state: packets.length ? "review" : "complete",
        reason: response.reason || failedSources[0]?.reason || "", message: "", object_review_proposals: [] });
      if (packets.length > 0) {
    ...
    const pendingCanonicalReviews = (sourcePath = "") => batchJobStore.listPlanSnapshots()
      .flatMap(snapshot => [snapshot, ...(snapshot.history || []).slice().reverse()]
        ...
        .map(review => ({ job_id: snapshot.job_id, review })))
      .filter((entry, index, entries) => entry.review?.item && entries.findIndex(other =>
        (other.review?.packet?.packet_hash || `${other.job_id}:${other.review?.item?.review_id}`)
    ...
    const legacyMigrationNotes = () => (appRef.vault.getMarkdownFiles ? appRef.vault.getMarkdownFiles() : [])
    ...
    const scanExistingZetaMigration = async () => {
      const decisions = [];
      for (const file of legacyMigrationNotes()) {
        const text = String(await appRef.vault.cachedRead(file) || "");
        if (/\nschema_version:\s*["']?2/u.test(text) || /\ncanonical_id:/u.test(text)) continue;
        decisions.push(Object.freeze({
          decision_id: `migration_${llmWikiHash.sha256(file.path).slice(0, 12)}`,
          kind: "update",
          path: file.path,
          title: String(file.basename || file.path.split("/").pop().replace(/\.md$/u, "")),
        }));
      }
      migrationScan = Object.freeze({
        status: decisions.length ? "review" : "no_change",
      ...
      return { ok: true, status: migrationScan.status, migration: migrationScan, provider_calls: 0,
    ...
    const lifecycleSnapshot = () => {
      ...
      const pendingCanonical = pendingCanonicalReviews(prodigyWikiController.getSnapshot().source?.path || "");
      ...
      return {
        ...
        ...(questionReviewRunId && snapshot.status === "review" && snapshot.risk_packets?.some(packet => packet.run_id === questionReviewRunId)
          ? { status: "review", prodigy_wiki: null, golden_wiki: null, source_question_sent: true, comparison_status: "not_checked",
            question_review_note: "원문 기반 요약·추론 제안입니다. 기존 관련 지식과 충돌은 자동 비교하지 않았습니다. 아래 충돌 상태는 등록된 충돌만 표시합니다. 제안 동작은 지식 후보 생성이며 정식 Knowledge 승격은 별도 승인 대상입니다." } : {}),
    ...
    const dispatchStartupIntent = async (intent) => {
      ...
      if (intent.action === "open_golden_review") {
        const { source, range } = prodigyWikiController.getSnapshot();
        const file = source && appRef.vault.getAbstractFileByPath(source.path);
        if (!file || llmWikiHash.sha256(await appRef.vault.read(file)) !== source.content_hash) {
          prodigyWikiController.dispatch({ type: "source_changed" });
          return { ok: false, reason: "source_revision_changed", provider_calls: 0 };
        }
        const retained = batchJobStore.listPlanSnapshots().find(row => row.plan?.source?.source_path === source.path
          && row.source_revision === source.content_hash
          && (range ? row.plan.source.scope?.start === range.start && row.plan.source.scope?.end === range.end
            && row.plan.source.scope?.scope_id === range.scope_id : !row.plan.source.scope));
        if (retained) {
          const result = await openStoredDocumentPlan(retained.job_id, { render: false });
          if (!result.ok) return result;
          const pending = pendingCanonicalReviews(source.path).find(row => row.job_id === retained.job_id);
          const item = pending?.review.item || pagePlanReviewItems.find(row => row.plan_kind === "compiled_document" && row.compiled_kind !== "source_guide" && !row.review_blocked);
          if (item) return KnowledgeExplorerHub.openCanonicalDocumentReview({ ...item, processing_job_id: retained.job_id });
          refreshWikiWorkspace("pending");
          return result;
        }
    ...
    const dispatchLifecycleAction = async (intent) => {
      ...
      // The review opener already awaited its exact surface. Do not replace it
      // with an unawaited second mount after returning the handoff result.
      if (intent.action !== "open_golden_review" || response?.status !== "waiting_for_human_review") llmWikiLifecycle.update(lifecycleSnapshot());
    ...
    // M1 phase 2/3: 변경안 검토 / 마이그레이션 승인 both route to the one prepared
    // canonical review whose proposal already targets this legacy note. There is no
    // separate migration approval surface: the review modal's exact approval is the
    // authorization. A legacy note with no covering analysis cannot be adopted
    // locally (that would need claims/evidence that do not exist), so it is answered
    // honestly instead of inventing a packet.
    const openMigrationReview = async (decisionId) => {
      const unavailable = () => ({ ok: false, status: "blocked", reason: "migration_review_unavailable", provider_calls: 0,
        write_counts: { canonical: 0, audit: 0, refresh: 0, git: 0 } });
      const decision = (migrationScan?.decisions || []).find((row) => row.decision_id === decisionId);
      if (!decision) return unavailable();
      const covering = workspaceReviewEntries().find(({ item }) => item?.proposed_target?.path === decision.path)
        || pendingCanonicalReviews().map(({ job_id, review }) => ({ item: { ...review.item, processing_job_id: job_id }, review }))
          .find(({ review }) => review.item?.proposed_target?.path === decision.path || review.packet?.target_path === decision.path);
      if (!covering) return unavailable();
      const opened = await openCanonicalDocumentReview(covering.item, { modal: true });
      return { ...opened, provider_calls: 0, migration_path: decision.path };
    };
    const refreshWikiPendingCount = () => {
      const canonical = workspaceReviewEntries().filter(row => !reviewClosed(row.status));
    ...
    renderInlineWikiReview = (host, snapshot) => {
      ...
      const selected = entries.find(row => row.item.review_id === activeWikiReviewId) || entries.find(row => !reviewClosed(row.status));
      if (!selected) return false;
      activeWikiReviewId = selected.item.review_id;
      wikiWorkspace.setProposals(entries.filter(row => !reviewClosed(row.status) || row.item.review_id === activeWikiReviewId).map(row => ({ id: row.item.review_id, title: row.item.title,
    ...
    knowledgeReviewWorkbench = window.KnowledgeExplorerController.mountKnowledgeReviewWorkbench({
      ...
      onQualityRetry: () => compileDocumentPlan({ allow_quality_retry: true }),
      qualityRetry: planQualityRetryState,
      onCanonicalReview: openCanonicalDocumentReview,
    ...
    const collectWikiReadSnapshot = async () => {
      const collected = { assets: dataSource.index(P.collectRecords(dataSource, dvRef)).assets,
        candidates: await window.KnowledgeCandidateStore.listCandidates(appRef, { status: "active" }), registry: window.KnowledgeExplorerRegistry };
      const selected = prodigyWikiController.getSnapshot().source;
      const file = selected && appRef.vault.getAbstractFileByPath(selected.path);
      if (!file) return collected;
      return window.LLMWikiWikiReadAdapter.buildSelectedSourceSnapshot({ ...collected,
        source_id: `source_${llmWikiHash.sha256(selected.path).slice(0, 24)}`, source_path: selected.path,
        source_text: await appRef.vault.read(file), content_hash: selected.content_hash,
        metadata: appRef.metadataCache?.getFileCache(file)?.frontmatter || {},
      });
    };
    ...
    llmWikiWikiSurface = window.LLMWikiWikiSurface.mountLlmWikiWikiSurface({
      app: appRef,
      conversationSession: llmWikiSession.conversationSession,
      getSelectedSource: () => prodigyWikiController.getSnapshot().source,
      onOpenCitation: (citation) => openGoldenCitation({ ...citation, evidence_quote: citation.excerpt, locators: [citation.locator] }),
      openQuestionReview: async (proposal, callbacks) => {
        if (proposal.contains_verified_context) return { ok: false, reason: "original_source_provenance_required" };
        if (proposal.document_body && proposal.grounded_claims?.length && (proposal.uses_canonical_provenance === true || (proposal.user_context || []).length > 0 || new Set(proposal.grounded_claims.flatMap(claim => claim.citations.map(c => c.source_id))).size > 1)) {
          return openCanonicalDocumentReview(proposal, callbacks);
        }
        const handoff = await window.LLMWikiWikiReadService.handoffQuestionProposal({
          app: appRef, proposal, materializer: inboxProposalMaterializer, controller: llmWikiRunController,
        });
        if (!handoff.ok) return handoff;
        questionReviewRunId = proposal.proposal_bundle.run_id;
        llmWikiLifecycle.update(lifecycleSnapshot());
        tabs.select("llmwiki");
        llmWikiLifecycleFrame.scrollIntoView({ block: "start" });
        return handoff;
      },
