(function (root) {
  ...
  async function saveProgramObject(app, program, options) {
    ...
    if (opts.captureRequest) {
      const runtime = root.CaptureActionRuntime || (typeof require === "function" ? require("./capture-action-runtime.js") : null);
      const writer = root.CaptureAuthorizedWriter || (typeof require === "function" ? require("./capture-authorized-writer.js") : null);
      const currentRevision = file ? runtime.sha256(await app.vault.read(file)) : null;
      if (opts.captureRequest.target_path !== path || opts.captureRequest.payload_hash !== runtime.hashPayload(path, normalized)) writer.assertCanonicalWriteRequest(opts.captureRequest, currentRevision);
      writer.assertCanonicalWriteRequest(opts.captureRequest, currentRevision);
    } else if (!file) {
      throw new Error("Workout program creation requires Capture writer authority.");
    }
