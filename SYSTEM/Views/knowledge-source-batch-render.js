(function (root) {
  ...
  function renderRows(parent, controller, current) {
    ...
    current.rows.forEach((row, index) => {
      ...
      const section = createEl(list, "section", { attr: { class: "knowledge-source-batch-row", "data-status": row.status, "aria-label": `자료 ${index + 1}: ${statusLabel(row)}` } });
      createEl(section, "h3", { text: `자료 ${index + 1}` });
      createEl(section, "p", { text: `${statusLabel(row)} · ${row.source_url}`, attr: { class: "knowledge-source-batch-status" } });
      checkbox(section, `selected-${row.item_id}`, `자료 ${index + 1} 저장`, "이 자료 저장", row.selected, locked, (value) => controller.updateRow(row.item_id, { selected: value }));
      labeledControl(section, "자료 제목", { name: `source_title-${row.item_id}`, value: row.source_title, required: true, disabled: locked }, (value) => controller.updateRow(row.item_id, { source_title: value }));
      if (row.status === "fallback" || row.status === "error" || row.status === "cancelled") {
        createEl(section, "p", { text: row.row_error || "사용자 텍스트 또는 메모를 입력해 주세요.", attr: { class: "knowledge-source-batch-notice" } });
      ...
      checkbox(section, `include-reusable-${row.item_id}`, `자료 ${index + 1} 재사용 가능한 지식으로 남기기`, "재사용 가능한 지식으로 남기기", row.include_reusable, locked, (value) => controller.updateRow(row.item_id, { include_reusable: value }));
      ...
      checkbox(section, `candidate-${row.item_id}`, `자료 ${index + 1} 후보도 만들기`, "후보도 만들어 검토 대기에 추가", row.create_candidate, row.pending || Boolean(row.candidate) || current.operation !== "idle" || current.saving, (value) => controller.updateRow(row.item_id, { create_candidate: value }));
      if (row.source) createEl(section, "p", { text: `저장된 자료: ${row.source.link}`, attr: { class: "knowledge-source-batch-success" } });
      ...
      if (row.candidate_error) {
        const retry = createEl(section, "button", { text: "후보 만들기 다시 시도", attr: { type: "button", "aria-label": `자료 ${index + 1} 후보 만들기 다시 시도` }, disabled: row.pending || current.operation !== "idle" || current.saving });
  ...
  function mountSourceBatchView(parent, controller) {
    if (!parent || !controller || typeof controller.state !== "function") throw new Error("자료 묶음 controller가 필요합니다.");
    ...
    function render() {
      ...
      const section = createEl(parent, "section", { attr: { class: "knowledge-source-batch", role: "dialog", "aria-label": "오늘의 자료 묶음", "aria-busy": current.operation !== "idle" || current.saving ? "true" : "false" } });
      createEl(section, "h2", { text: "오늘의 자료 묶음" });
      ...
      const form = createEl(section, "fieldset", { attr: { class: "knowledge-source-batch-fields", "aria-label": "자료 묶음 입력" }, disabled: current.operation !== "idle" || current.saving });
      const urls = labeledControl(form, "자료 URL 목록", { name: "urls_text", value: current.values.urls_text, tag: "textarea", required: true }, (value) => controller.setValues({ urls_text: value }));
      createEl(form, "p", { text: "한 줄에 하나씩, 최대 20개", attr: { class: "knowledge-source-batch-hint" } });
      createEl(form, "label", { text: "자료 유형", attr: { for: "source_kind" } });
      const kind = createEl(form, "select", { attr: { name: "source_kind", "aria-label": "자료 유형", class: "knowledge-source-batch-input" } });
  ...
  function renderActions(section, controller, current) {
    ...
    const check = createEl(actions, "button", { text: "자료 목록 확인", attr: { type: "button", "aria-label": "자료 목록 확인" }, disabled: current.operation !== "idle" || current.saving });
    check.onclick = () => controller.prepare();
    const retrieve = createEl(actions, "button", { text: current.operation === "retrieving" ? "가져오기 취소" : "기사 가져오기", attr: { type: "button", "aria-label": current.operation === "retrieving" ? "가져오기 취소" : "기사 가져오기" }, disabled: current.saving || (!current.rows.length && current.operation !== "retrieving") });
    ...
    const save = createEl(actions, "button", { text: current.saving ? "저장 중" : "선택한 자료 저장", attr: { type: "button", "aria-label": "선택한 자료 저장" }, disabled: current.operation !== "idle" || current.saving || !current.rows.length });
