...
test("Given the Workout hub, When it loads modules, Then it uses the shared hub loader instead of a private loader", () => {
  const hub = readWorkoutHub();

  assert.match(hub, /SYSTEM\/Views\/prodigy-hub-loader\.js/);
  assert.match(hub, /ProdigyHubLoader\.loadManifest/);
