...
test("grounded reusable singleton becomes its own reviewable page",()=>{const input=inventory();const singleton={...claim(9,"외부 분류",["c9"]),text:"규제 변화는 별도 검토가 필요하다."};const result=planner.plan({inventory:{...input,claims:[...input.claims,singleton]}});assert.equal(result.ok,true);assert.equal(result.value.topic_pages.flatMap((page)=>page.claim_ids).includes(singleton.claim_id),true);assert.equal(result.value.source_only_claim_ids.includes(singleton.claim_id),false);const used=result.value.topic_pages.flatMap((page)=>page.claim_ids).concat(result.value.source_only_claim_ids);assert.equal(used.filter((id)=>id===singleton.claim_id).length,1);});
test("deterministic fallback forms exact-topic pages including grounded singletons",()=>{const result=planner.plan({inventory:inventory()});assert.equal(result.ok,true);assert.deepEqual(result.value.topic_pages.map((p)=>p.title).sort(),["세무","유치권","직영 건축"]);assert.deepEqual(result.value.source_only_claim_ids.length,0);assert.equal(result.writer_count,0);});
test("unassigned source summaries land in source_only instead of vanishing",()=>{const input=inventory();const extra={...claim(9,"외부 분류",["c9"]),role:"source_summary",text:"외부 자료의 단순 인용이다."};const rows=[...input.claims,extra];const result=planner.plan({inventory:{...input,claims:rows}});assert.equal(result.ok,true);const all=result.value.topic_pages.flatMap((page)=>page.claim_ids).concat(result.value.source_only_claim_ids);assert.equal(new Set(all).size,rows.length);assert.equal(result.value.source_only_claim_ids.includes(extra.claim_id),true);});
