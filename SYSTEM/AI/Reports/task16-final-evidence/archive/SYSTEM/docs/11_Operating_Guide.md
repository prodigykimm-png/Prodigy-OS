...
Morning Brief Context  (buildMorningBriefContext)
    ↓
Home Mission Control:
  Workspace Shortcuts → Morning Brief → Today's Focus → Continue → Micro Log
  → [collapsed] Needs Attention → Quick Actions → Todoist → Workspace Launcher → System Status
```
...
변환 상태는 `capture_started` → `ai_proposal` → `human_review` → `human_confirmed` → `object_committed`이고 terminal outcome은 `rejected`, `cancelled`, `no_change`, `stale`, `conflict`, `error`다. 첫 trusted action은 exact proposal을 검토 화면에 남기며 저장하지 않는다. 이후 같은 review의 Confirm action만 한 번 쓰기를 시도한다. People의 명시적 수동 memo/interaction/edit/delete, 승인된 existing-person insight append, Workout의 수동 operational edit은 일상 운영 carve-out이다. Missing-person creation과 program/replacement/running/nutrition import는 carve-out이 아니며 Capture proposal이다.
...
### 지식 작성과 문헌노트 작성 — 두 개의 직접 입력 경로

`HUB/50 Knowledge.md` 상단의 **+ 지식 작성**과 **+ 문헌노트 작성**은 Daily Reflection이나 Reading 없이도 지식과 문헌노트를 직접 입력하는 경로다.
...
- **+ 문헌노트 작성**: 기사·칼럼·YouTube·인강·논문·공식 문서를 `literature_note` Resource로 저장한다. `문헌노트 한 건`과 `문헌노트 묶음` 경로가 있다.
  - **문헌노트 한 건**: URL(선택), 제목, 출처 주장, 내 해석, 재사용 가능한 지식을 직접 입력한다.
  - **문헌노트 묶음**: 1~20개 URL을 넣고 `공개 자료 가져오기`를 누르면 공개 페이지 텍스트를 가져와 AI가 항목별 요약을 만든다. 각 항목에 사람의 한 줄 판단이 필수이며, 없으면 저장되지 않는다.
- 두 경로 모두 저장 후 `검증 대기`로 들어가며, AI가 Candidate나 Knowledge를 자동 생성·승인하지 않는다.
- **검증 대기 열기/닫기**: `HUB/50 Knowledge.md`의 지식 구축 탭에서 필요할 때만 승인 전 후보를 펼쳐 제목·지식 문장·도메인·주제를 사람이 확인한 뒤 승인·보류·반려한다.
...
저장 형식 계약은 `DAILY/`, `PARA/`, `ZETA/`, `SYSTEM/AI/Memory/`, `.llmwiki-audit/`의 일반 파일이다. `.llmwiki-derived/`와 `SYSTEM/CACHE/`는 원본이 아닌 재생성 가능한 파생 캐시이므로 백업 대상이 아니다. 백업 도구는 저장소 루트나 실제 Vault를 받지 않으며, 루트에 `.prodigy-disposable-fixture` 표식이 있는 임시 fixture 복사본만 허용한다. 심볼릭 링크, 절대/상위 경로, 중복 경로, 추가 payload, manifest 변조, payload SHA-256 불일치는 복원 전에 실패한다.
...
이 명령은 release fixture의 disposable 복사본을 만들고, 저장 형식별 합성 파일을 추가한 다음 아래 절차를 실제로 실행한다.

1. 정렬된 canonical `path + SHA-256` manifest와 payload 백업 생성
2. disposable 원본 파일 변조 및 삭제
3. 별도 백업 payload 변조 후 복원 시도 (`backup_payload_tampered`로 실패해야 함)
4. 정상 백업 복원 및 원래 canonical manifest와 byte-for-byte 비교
5. source, backup, tampered backup, restore 상위 임시 루트 전부 삭제

개별 계약 명령은 다음과 같다. `<source>`와 `<destination>`은 모두 표식이 있는 disposable fixture 루트여야 한다.

```bash
node SYSTEM/CI/repository-data-backup.js manifest <source>
node SYSTEM/CI/repository-data-backup.js create <source> <backup-root>
node SYSTEM/CI/repository-data-backup.js restore <backup-root> <destination>
```

정상 드릴 receipt의 `before`와 `restored` 배열, `canonical_set_sha256`, `backup_manifest_sha256`, `negative.reason`, `cleanup.all_roots_deleted`를 확인한다. `negative.reason`은 정확히 `backup_payload_tampered`, cleanup은 `true`여야 한다. Release gate는 이 드릴과 여섯 recovery injection을 별도 명령으로 실행하므로 파일이나 suite를 삭제하면 preflight 또는 직접 테스트가 RED가 된다.
