...
const OUTER_SCROLL_OWNER_RESOLVER = `()=>{const visible=node=>{const box=node.getBoundingClientRect(),style=getComputedStyle(node),excluded=node.closest('[hidden],[inert],[aria-hidden="true"]');return node.isConnected&&!excluded&&box.width>0&&box.height>0&&style.display!=='none'&&style.visibility!=='hidden'},describe=node=>{const style=getComputedStyle(node);return{tag:node.tagName.toLowerCase(),className:String(node.className),id:node.id||null,overflowY:style.overflowY,clientHeight:node.clientHeight,scrollHeight:node.scrollHeight,scrollTop:node.scrollTop}},shells=[...document.querySelectorAll('.prodigy-app-shell[data-workspace-id="knowledge"]')].filter(visible);if(shells.length!==1)throw new Error('TASK15_OUTER_SCROLL_SHELL_COUNT:'+shells.length);const shell=shells[0],candidates=[shell,...shell.querySelectorAll('*')];for(let node=shell.parentElement;node&&node!==document.body&&node!==document.documentElement;node=node.parentElement)candidates.push(node);const unique=[...new Set(candidates.filter(Boolean))],owners=unique.filter(node=>{const style=getComputedStyle(node);return visible(node)&&/(auto|scroll)/u.test(style.overflowY)&&node.scrollHeight>node.clientHeight+1});if(owners.length===0)throw new Error('TASK15_OUTER_SCROLL_ZERO:'+JSON.stringify(unique.map(describe)));if(owners.length!==1)throw new Error('TASK15_OUTER_SCROLL_AMBIGUOUS:'+JSON.stringify(owners.map(describe)));if(!owners[0].classList.contains('prodigy-app-shell-body'))throw new Error('TASK14_SCROLL_OWNER_NOT_APPSHELL_BODY:'+JSON.stringify(describe(owners[0])));return owners[0]}`;
...
async function assertOuterScrollResolverMutationSensitive(harness) {
  const result = await harness.evaluate(`(()=>{const resolveOuterScrollOwner=${OUTER_SCROLL_OWNER_RESOLVER},describe=node=>({tag:node.tagName.toLowerCase(),className:String(node.className),id:node.id||null,overflowY:getComputedStyle(node).overflowY,clientHeight:node.clientHeight,scrollHeight:node.scrollHeight}),owner=resolveOuterScrollOwner(),activeShell=owner.closest('.prodigy-app-shell[data-workspace-id="knowledge"]'),ownerStyle=owner.getAttribute('style'),synthetic=document.createElement('div'),content=document.createElement('div'),hiddenOwner=document.createElement('div'),hiddenContent=document.createElement('div'),inertOwner=document.createElement('div'),inertContent=document.createElement('div'),messages=[];synthetic.id='task15-synthetic-scroll-owner';synthetic.style.cssText='display:block;block-size:100px;inline-size:100%;overflow-y:auto';content.style.blockSize='3000px';synthetic.append(content);for(const [ignored,child,kind] of [[hiddenOwner,hiddenContent,'hidden'],[inertOwner,inertContent,'inert']]){ignored.id='task15-'+kind+'-scroll-owner';ignored.style.cssText='block-size:100px;inline-size:100%;overflow-y:auto';child.style.blockSize='3000px';ignored.append(child);if(kind==='hidden')ignored.hidden=true;else ignored.inert=true}try{owner.style.setProperty('overflow-y','hidden','important');try{resolveOuterScrollOwner()}catch(error){messages.push(String(error.message))}if(ownerStyle===null)owner.removeAttribute('style');else owner.setAttribute('style',ownerStyle);activeShell.append(hiddenOwner,inertOwner);const hiddenAndInertIgnored=resolveOuterScrollOwner()===owner;activeShell.append(synthetic);try{resolveOuterScrollOwner()}catch(error){messages.push(String(error.message))}return{before:[describe(owner)],zeroError:messages[0]||'',hiddenAndInertIgnored,ignoredOwners:[describe(hiddenOwner),describe(inertOwner)],mutationOwners:[describe(owner),describe(synthetic)],ambiguousError:messages[1]||''}}finally{synthetic.remove();hiddenOwner.remove();inertOwner.remove();if(ownerStyle===null)owner.removeAttribute('style');else owner.setAttribute('style',ownerStyle)}})()`);
  assert.match(result.zeroError, /^TASK15_OUTER_SCROLL_ZERO:/u, `removing ownership must fail explicitly: ${JSON.stringify(result)}`);
  assert.equal(result.hiddenAndInertIgnored, true, `hidden and inert owners must not false-positive: ${JSON.stringify(result)}`);
  assert.match(result.ambiguousError, /^TASK15_OUTER_SCROLL_AMBIGUOUS:/u, `adding a second owner must fail explicitly: ${JSON.stringify(result)}`);
  return result;
}
...
test("protected real Obsidian mounts Knowledge with true 200% compact AppShell reflow",  { timeout: 240000 }, async () => {
  ...
  try {
    ...
    mutationEvidence = await assertOuterScrollResolverMutationSensitive(harness);
  } finally {
    if (harness) {
