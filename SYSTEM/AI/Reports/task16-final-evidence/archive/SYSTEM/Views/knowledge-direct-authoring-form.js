(function (root) {
  ...
  function pickerField(parent, config) {
    ...
    const trigger = wrap.createEl("button", { text: "", attr: {
      ...
      "aria-label": config.label, title: "클릭해 목록 열기", "aria-haspopup": "listbox", "aria-expanded": "false", class: "prodigy-configurator-chip",
      style: "display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;min-height:44px;box-sizing:border-box;text-align:left;white-space:normal;cursor:pointer;"
    } });
    ...
    const menuStyle = "position:absolute;z-index:20;left:0;right:0;bottom:calc(100% + 4px);padding:8px;border:1px solid var(--background-modifier-border);background:var(--background-primary);";
    const menuMount = wrap.createEl("div", { attr: { class: "knowledge-direct-picker-menu prodigy-utility-card", style: `${menuStyle}display:none;` } });
    ...
    function renderOptions(optionsMount) {
      ...
      visible.forEach((option) => {
        ...
        const item = optionsMount.createEl("button", { text: `${isSelected ? "✓ " : ""}${option.label}`, attr: {
          type: "button", "data-picker-option": option.value, role: "option", "aria-selected": isSelected ? "true" : "false", class: "prodigy-btn prodigy-btn-quiet",
    ...
    function render() {
      ...
      const search = menuMount.createEl("input", { attr: {
        type: "search", "data-picker-search": config.name, placeholder: "검색…", "aria-label": `${config.label} 검색`, class: "prodigy-configurator-chip",
    ...
    render();
    if (config.help) parent.createEl("p", { text: config.help, attr: { id: `knowledge-direct-${config.name}-help`, style: "margin:4px 0;color:var(--text-muted);" } });
    return trigger;
  }
  function field(parent, config) {
    ...
    const input = parent.createEl(config.rows ? "textarea" : "input", { attr: {
      ...
      "aria-describedby": config.help ? `knowledge-direct-${config.name}-help` : undefined, class: "prodigy-configurator-chip",
