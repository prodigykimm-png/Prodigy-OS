...
function main() {
  ...
  assert.equal(responsive.compactMax, tokens.RESPONSIVE_BREAKPOINTS.compactMax);
  assert.equal(responsive.mediumMin, tokens.RESPONSIVE_BREAKPOINTS.compactMax + 1);
  assert.equal(responsive.wideMin, tokens.RESPONSIVE_BREAKPOINTS.utilityTwoColumnMax + 1);
  ...
  const compactListPane = compactHost.querySelector(".ppw-list-pane");
  assert.equal(compactListPane.isConnected, true);
  assert.equal(compactHost.querySelector(".ppw-detail-pane"), null);
  compactWorkspace.selectPerson("PARA/RESOURCES/CONTACTS/QA-THROWAWAY-반응형.md");
  assert.equal(compactListPane.isConnected, false);
  assert.ok(compactHost.querySelector(".ppw-detail-pane"));
  ...
  assert.match(compactModal[0], /inline-size:\s*100%/);
  assert.match(compactModal[0], /max-inline-size:\s*100%/);
