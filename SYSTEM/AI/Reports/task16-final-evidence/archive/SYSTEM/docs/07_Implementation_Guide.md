...
Capture writer를 연결하는 구현은 public 상태 `capture_started` → `ai_proposal` → `human_review` → `human_confirmed` → `object_committed`와 terminal outcome `rejected`, `cancelled`, `no_change`, `stale`, `conflict`, `error`만 사용한다. 첫 trusted event는 exact review를 렌더하고 zero authorization/zero write로 끝나야 한다. 같은 mount/session/review의 이후 Confirm event만 single-use canonical request를 만들며, adapter는 mutation boundary에서 즉시 expected revision을 대조하고 write 후 path+reread+SHA-256을 검증한다.
...
개발자와 GitHub Actions는 Node.js 24 및 Python 3.12/uv를 사용하고 동일한 단일 command를 실행한다.

```bash
bash SYSTEM/CI/run-release-gate.sh
```

The command above is portable Node 24/Ubuntu evidence; macOS capability rows are reported as not applicable. Final release authority must run on macOS with Aside:

```bash
bash SYSTEM/CI/run-final-release.sh
```

The final workflow requires freshly executed responsive/real-Obsidian visual256 and journeys8 artifacts, sanitizes persisted text copies with a raw/persisted byte-and-command-result redaction manifest, recomputes archive and gate-log digests through explicit temporary paths, verifies the derived receipt against the sanitized retained copy, and cleans only the disposable execution paths. The receipt is post-projection derived evidence, so excluding it from the raw projection prevents a self-referential hash cycle. Width 390 is narrow desktop evidence and never physical-iPhone proof; physical iPhone remains `not_tested` / `not_proven`.

이 runner는 모든 View syntax check, direct JavaScript/Python test, Property audit, stability smoke, consolidation audit를 실행한다. 각 command의 stdout/stderr를 그대로 보존하고, 실패 후에도 남은 command를 계속 실행한 뒤 하나라도 실패하면 nonzero로 종료한다. `--help`는 usage를 출력하고 `--self-test`는 synthetic command의 stdout/stderr와 exit propagation을 검증한다.

Generated audit output은 tracked fixture에 쓰지 않는다. GitHub Actions에서는 `$RUNNER_TEMP`, 로컬에서는 임시 directory를 사용한다. Consolidation audit 입력은 SHA-256 manifest로 고정된 tracked `SYSTEM/CI/fixtures/consolidation/` fixture이며, 필수 입력이 없거나 malformed이면 audit은 fail closed한다.
