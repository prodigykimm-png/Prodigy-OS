...
function htmlCollection(owner) {
  return new Proxy({
    item(index) { return owner._children[index] || null; },
    get length() { return owner._children.length; },
    [Symbol.iterator]() { return owner._children[Symbol.iterator](); }
  }, {
    get(target, key, receiver) {
      if (typeof key === "string" && /^\d+$/.test(key)) return owner._children[Number(key)];
      return Reflect.get(target, key, receiver);
    }
  });
}

class HtmlCollectionRoot extends FakeElement {
  get children() { return htmlCollection(this); }
  set children(value) { this._children = Array.from(value || []); }
  ...
  empty() { this._children = []; this.text = ""; }
}
...
function testHtmlCollectionTraversalContract() {
  const root = new HtmlCollectionRoot("section");
  const liveChildren = root.children;
  assert.equal(typeof liveChildren.forEach, "undefined", "HTMLCollection-equivalent must not inherit Array.forEach");
  root.createEl("span");
  assert.equal(liveChildren.length, 1, "collection must reflect live child insertion");
  root.empty();
  assert.equal(liveChildren.length, 0, "collection must reflect live child removal");
  const shell = view.mountKnowledgeExplorer({ container: root, model: createModel(), logicalWidth: 1024 });
  assert.equal(shell.container, root);
  assert.ok(root.children.length > 0);
  shell.dispatch({ type: "set-query", query: "" });
  assert.ok(root.children.length > 0, "rerender must traverse a live collection deterministically");
  shell.destroy();
}
...
async function main() {
  testHtmlCollectionTraversalContract();
