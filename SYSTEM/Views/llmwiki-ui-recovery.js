(function (root) {
  ...
  const COPY = Object.freeze({
    promotion_review_required: { copy: "상충이나 필수 검토 사항이 남아 있습니다. 변경안과 근거는 유지되며 문서에는 적용하지 않았습니다. 남은 조건과 근거를 확인해 다시 검토해 주세요.", action: "review_sources" },
    source_not_in_corpus: { copy: "선택한 원문을 검색 대상으로 읽지 못했습니다. 자료를 다시 선택해 주세요.", action: "select_sources" },
    source_privacy_blocked: { copy: "선택한 원문은 전송 허용 대상이 아닙니다.", action: "select_sources" },
    source_revision_changed: { copy: "원문이 변경됐습니다. 이전 답변과 인용은 당시 원문 기준으로 남겨 두었습니다. 변경된 자료를 명시적으로 다시 선택해 질문해 주세요.", action: "select_sources" },
    invalid_scope_type: { copy: "검색 범위를 확인하지 못했습니다. 읽기 모드를 다시 선택해 주세요.", action: "review_sources" },
    corpus_empty: { copy: "검색할 자료가 없습니다. 원문을 선택해 주세요.", action: "select_sources" },
    no_relevant_evidence: { copy: "질문에 답할 근거가 없습니다.", action: "enter_query" },
    invalid_query: { copy: "질문을 1~2000자로 입력해 주세요.", action: "enter_query" },
    provider_executable_not_found: { copy: "AI 실행 파일을 찾지 못했습니다. Runtime 설정을 확인해 주세요.", action: "open_settings" },
    provider_transport_error: { copy: "AI 연결에 실패했습니다. 입력과 이전 대화는 유지되며 이번 답변은 추가되지 않았습니다. 연결을 확인하고 실패한 질문을 다시 시도해 주세요.", action: "retry" },
    provider_response_parse_failed: { copy: "AI 응답을 읽지 못했습니다. 입력과 이전 대화는 유지되며 이번 응답은 지식으로 저장되지 않았습니다. 실패한 질문을 명시적으로 다시 시도해 주세요.", action: "retry" },
    provider_schema_invalid: { copy: "AI 응답 형식이 계약을 충족하지 못했습니다. 입력과 이전 대화는 유지되며 잘못된 응답은 적용되지 않았습니다. 실패한 질문을 명시적으로 다시 시도해 주세요.", action: "retry" },
    retry_required: { copy: "중단된 작업입니다. 명시적으로 다시 시도해 주세요.", action: "retry" },
    retry_not_available: { copy: "다시 시도할 중단 작업이 없습니다. 현재 작업 상태를 확인해 주세요.", action: "review_sources" },
    retry_claim_failed: { copy: "재시도 작업을 확보하지 못했습니다. 다시 시도할 수 있습니다.", action: "retry" },
    proposal_validation_failed: { copy: "초안 근거 검증에 실패했습니다. 답변은 남겨 두었으며 초안은 적용하지 않았습니다. 출처를 확인한 뒤 다시 검토를 요청해 주세요.", action: "review_sources" },
    review_handoff_failed: { copy: "검토 화면을 준비하지 못했습니다. 답변과 준비된 초안은 유지되며 승인이나 저장은 진행하지 않았습니다. 검토 전달 다시 시도를 눌러 주세요.", action: "retry" },
    existing_review_must_resolve_first: { copy: "진행 중인 검토를 먼저 마무리해 주세요.", action: "review_sources" },
    ...
    blocked: { copy: "분석이 중단되었습니다. 명시적으로 다시 시도해 주세요.", action: "retry_later" },
    ...
    source_unavailable: { copy: "원문 근거를 확인할 수 없습니다. 이전 인용을 다른 자료에 연결하지 않았습니다. 자료의 존재와 선택 범위를 확인해 주세요.", action: "select_sources" },
