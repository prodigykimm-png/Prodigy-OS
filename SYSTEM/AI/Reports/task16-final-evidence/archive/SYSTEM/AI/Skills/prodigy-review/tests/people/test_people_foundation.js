...
function main() {
  ...
  // Preparation is deterministic and write-free. Capture-authorized execution,
  // duplicate/conflict rejection, and direct-call mutation coverage live in
  // test_people_creation_authority.js so this foundation test cannot pass via
  // the Store's direct-writer fail-closed behavior.
  return store.preparePeopleCreation(fakeApp, "새 사람").then((result) => {
    ...
    assert.equal(created.length, 0, "preparing a People proposal must not write");
  }).then(async () => {
    ...
    const personalRequired = require(path.join(ROOT, "SYSTEM/AI/Skills/prodigy-review/tests/shared/fixtures/workspace-manifest-v1.json")).entries.personal.required;
    const peopleRuntime = [
    ...
    peopleRuntime.forEach((modulePath) => assert.ok(personalRequired.includes(modulePath), `${modulePath} must load for Personal`));
    peopleRuntime.slice(1).forEach((modulePath, index) => {
      assert.ok(personalRequired.indexOf(modulePath) > personalRequired.indexOf(peopleRuntime[index]), `${modulePath} must load after ${peopleRuntime[index]}`);
    });
    ...
    // --- QuickAdd tracked release contract (never read ignored personal plugin state) ---
    const quickaddPath = "SYSTEM/AI/Skills/prodigy-review/tests/people/fixtures/quickadd-people-v1.json";
    const quickadd = JSON.parse(read(quickaddPath));
    assert.deepEqual(quickadd, {
      schema_version: 1,
      choices: [{
        name: "사람 추가",
        type: "Template",
        templatePath: "SYSTEM/TEMPLATE/FORMAT/template_people.md",
        folder: {
          enabled: true,
          folders: ["PARA/RESOURCES/CONTACTS"],
          chooseWhenCreatingNote: false,
          createInSameFolderAsActiveFile: false
        }
      }]
    });
    assert.equal(JSON.stringify(quickadd).includes("Create Contact"), false);
    assert.equal(JSON.stringify(quickadd).includes(".obsidian"), false);
    const releaseManifest = JSON.parse(read("SYSTEM/CI/release-gate-manifest.json"));
    assert.ok(releaseManifest.delivery.projected_paths.some((entry) => entry.path === quickaddPath), `${quickaddPath} must be included in the projected deliverable`);
