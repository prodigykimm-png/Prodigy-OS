...
test("Given an unsafe Node override, When the desktop runner resolves its executable, Then it falls back to a trusted candidate", () => {
  assert.equal(runner.resolveNodeCommand({ PRODIGY_NODE_BIN: "/tmp/untrusted-node", HOME: "/Users/tester", PATH: "" }, (candidate) => candidate === "/Users/tester/.hermes/node/bin/node"), "/Users/tester/.hermes/node/bin/node");
});
...
test("Given Obsidian's PATH does not expose npm, When the desktop runner prepares the child environment, Then Node's bin directory is available to the collector", () => {
  const nodeBin = "/Users/tester/.hermes/node/bin/node";
  const runtimePath = runner.runtimePath(nodeBin, { HOME: "/Users/tester", PATH: "/usr/bin" });
  assert.equal(runtimePath.split(":")[0], "/Users/tester/.hermes/node/bin");
