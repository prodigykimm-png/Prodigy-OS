 ...
 async function renderNutritionPanel(app, panel) {
   ...
   if (!nutrition || !healthStoreApi || !storeApi) throw new Error("Nutrition modules are unavailable.");
    ...
    function render() {
      ...
      const prevBtn = dateNav.createEl("button", { text: "←", attr: { class: "prodigy-btn workout-button workout-nav-btn", type: "button", "aria-label": "이전 날" } });
      dateNav.createEl("span", { text: selectedDate, attr: { class: "workout-nutrition-date-label" } });
      const nextBtn = dateNav.createEl("button", { text: "→", attr: { class: "prodigy-btn workout-button workout-nav-btn", type: "button", "aria-label": "다음 날" } });
      ...
      macros.forEach((m) => {
        const chip = summary.createDiv({ attr: { class: "workout-nutrition-chip prodigy-utility-card" } });
      ...
      const importBtn = actions.createEl("button", { text: "FatSecret CSV 가져오기", attr: { class: "prodigy-btn prodigy-btn-primary workout-button mod-cta", type: "button" } });
      ...
      onOpen() {
        ...
        input.onchange = async () => {
          ...
          try {
            ...
            const confirmBtn = actions.createEl("button", { text: `${result.entries.length}개 가져오기`, attr: { class: "prodigy-btn prodigy-btn-primary workout-button mod-cta", type: "button" } });
            confirmBtn.onclick = async () => {
              confirmBtn.disabled = true;
              confirmBtn.textContent = "가져오는 중…";
              try {
                const importedAt = new Date().toISOString();
                const seed = nutrition.buildImportReceipt({
                  import_id: result.entries[0] ? result.entries[0].import_id : "",
                  file_basename: file.name,
                  imported_at: importedAt
                });
                const outcome = await captureWriter.importNutrition(
                  store, nutrition, result.entries, result.warnings, seed,
                  captureRuntime.humanConfirmation("workout-nutrition-import", importedAt)
                );
                if (outcome.review_required) {
                  const review = outcome.capture.record;
                  captureRuntime.renderReview(previewArea, review, {
                    confirm: async () => { await captureWriter.importNutrition(store, nutrition, result.entries, result.warnings, seed, captureRuntime.humanConfirmation("workout-nutrition-import", importedAt), review); this.close(); if (onDone) onDone(); },
                    reject: () => { captureRuntime.decideHumanReview(review, captureRuntime.humanConfirmation("workout-nutrition-import", importedAt), "workout-nutrition-import", "reject"); this.close(); },
                    cancel: () => { captureRuntime.decideHumanReview(review, captureRuntime.humanConfirmation("workout-nutrition-import", importedAt), "workout-nutrition-import", "cancel"); this.close(); }
                  });
                  return;
                }
                const results = outcome.result.saved;
