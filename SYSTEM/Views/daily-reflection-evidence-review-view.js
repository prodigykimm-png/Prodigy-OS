(function (root) {
  ...
  function renderEvidenceCards(body, modal, dateStr, onNotice) {
    modal.proposal.evidence_blocks.forEach((block, index) => {
      const card = body.createEl("div", { attr: { class: "reflection-evidence-card" } });
      ...
      check.onchange = () => { check.checked ? modal.selectedIds.add(block.evidence_id) : modal.selectedIds.delete(block.evidence_id); modal.refreshApprovalFooter(); };
  ...
  function renderFooter(shell, modal, onConfirm) {
    ...
    const all = button(actions, "전체 선택"); all.onclick = () => { modal.selectedIds = new Set(modal.proposal.evidence_blocks.map((block) => block.evidence_id)); modal.render(); };
    const clear = button(actions, "선택 해제"); clear.onclick = () => { modal.selectedIds.clear(); modal.render(); };
    const back = button(actions, "다시 입력"); back.onclick = () => { modal.resetProposalSelection(); modal.phase = "input"; modal.render(); };
