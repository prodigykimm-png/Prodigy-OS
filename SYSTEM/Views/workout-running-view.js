    ...
    class RunImportModal extends ModalBase {
      ...
      showActivityPreview(area, activity, file, store, onDone) {
        ...
        [
          ...
          const chip = stats.createDiv({ attr: { class: "workout-running-stat" } });
          chip.createEl("span", { text: s.label, attr: { class: "workout-running-stat-label" } });
        ...
        const confirmBtn = actions.createEl("button", { text: "저장", attr: { class: "workout-button mod-cta", type: "button" } });
       ...
       this.contentEl.createEl("p", { text: "HKWorkoutActivityTypeRunning 항목만 가져옵니다. 위치 좌표는 저장되지 않습니다.", attr: { class: "workout-muted" } });
        ...
        input.onchange = async () => {
          ...
          try {
            ...
            const confirmBtn = actions.createEl("button", { text: `${result.activities.length}개 가져오기`, attr: { class: "workout-button mod-cta", type: "button" } });
            confirmBtn.onclick = async () => {
              confirmBtn.disabled = true;
