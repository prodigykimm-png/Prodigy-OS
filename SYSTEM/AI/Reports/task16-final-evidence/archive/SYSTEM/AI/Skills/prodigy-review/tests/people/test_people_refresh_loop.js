...
test("Personal forced repaint owns prior disposal before mount replacement", () => {
  const source = hubSource();
  const assertOwnedDisposal = (candidate) => {
    const dispose = candidate.indexOf('disposePriorWorkspace.call(priorWorkspaceApi)');
    const clear = candidate.indexOf("peopleMount.empty()", dispose);
    const replace = candidate.indexOf("workspaceApi = window.PeopleView.renderPeopleWorkspace", clear);
    assert.ok(dispose >= 0, "forced repaint must invoke prior workspace disposal");
    assert.ok(clear > dispose, "prior workspace must be disposed before its DOM is emptied");
    assert.ok(replace > clear, "replacement controller must mount after prior disposal and clear");
  };
  assertOwnedDisposal(source);
  const mutation = source.replace("disposePriorWorkspace.call(priorWorkspaceApi)", "void priorWorkspaceApi");
  assert.throws(() => assertOwnedDisposal(mutation), /must invoke prior workspace disposal/);
});
...
test("Given 189 people and six unchanged 2.5-second reruns, When Personal stays open, Then one mount retains user state and explicit refresh still repaints", async () => {
  ...
  const previousWorkspaceEvents = global.PeopleWorkspaceEvents;
  ...
  global.PeopleWorkspaceEvents = {
    create() {
      liveEventVectors += 1;
      const scope = workspaceEvents.create();
      let disposed = false;
  ...
  global.ResizeObserver = class {
    ...
    observe() { if (!this.connected) { this.connected = true; liveObservers += 1; } }
    disconnect() { if (this.connected) { this.connected = false; liveObservers -= 1; } }
  };
  global.PeopleStyles = {
  ...
  global.PeopleView = Object.assign({}, view, {
    ...
    renderPeopleWorkspace(options) {
      ...
      controllerCreates += 1;
      liveControllers += 1;
      const destroy = latestWorkspaceApi.destroy.bind(latestWorkspaceApi);
      let destroyed = false;
      latestWorkspaceApi.destroy = () => {
        if (destroyed) return;
        destroyed = true;
  ...
  global.ProdigyWorkspaceManifest = { get: () => ({ renderer: "personal" }) };
  global.ProdigyHubLoader = {
    mountWorkspace(_app, _manifest, options) {
      return Promise.resolve(options.renderers.personal({
        optional_ready: Promise.resolve(),
        onOptionalReady() {},
        retry() {},
        scope: { track(cleanup) { mountCleanups.push(cleanup); } }
      }));
    }
  };
  ...
  try {
    ...
    for (const trigger of ["create", "edit", "retry"]) {
      const prior = latestWorkspaceApi;
      await refresh();
      assert.equal(liveControllers, 1, `${trigger} repaint keeps exactly one live controller`);
      assert.equal(liveEventVectors, 1, `${trigger} repaint keeps exactly one listener/timer/observer ownership vector`);
      assert.equal(liveObservers, 1, `${trigger} repaint keeps exactly one ResizeObserver`);
      assert.equal(controllerDestroys, controllerCreates - 1, `${trigger} repaint destroys its prior controller before replacement`);
      assert.notEqual(latestWorkspaceApi, prior, `${trigger} repaint installs a fresh controller`);
    }
    ...
    assert.equal(renderCount, 4, "create/edit/retry refreshes must each repaint once");
    ...
    mountCleanups.slice().reverse().forEach((cleanup) => cleanup());
    assert.equal(liveControllers, 0, "final mount close destroys the last People controller");
    assert.equal(liveEventVectors, 0, "final mount close releases the listener/timer/observer ownership vector");
    assert.equal(liveObservers, 0, "final mount close disconnects the final ResizeObserver");
    assert.equal(controllerDestroys, controllerCreates, "every created People controller is destroyed exactly once");
    console.log(JSON.stringify({ ticks: 6, interval_ms: 2500, rows: 189, mounts: mountCount, unchanged_renders: 1, forced_renders: renderCount, index_touches: indexTouches, controllers_created: controllerCreates, controllers_destroyed: controllerDestroys, live_event_vectors_after_close: liveEventVectors, live_observers_after_close: liveObservers, live_after_close: liveControllers, state: expectedState, scroll_top: scrollOwner.scrollTop, focus_retained: true, tail_path: tailPath }));
  } finally {
    delete global.__prodigyPersonalRenderGuard;
    ...
    if (previousWorkspaceEvents === undefined) delete global.PeopleWorkspaceEvents; else global.PeopleWorkspaceEvents = previousWorkspaceEvents;
    if (previousResizeObserver === undefined) delete global.ResizeObserver; else global.ResizeObserver = previousResizeObserver;
